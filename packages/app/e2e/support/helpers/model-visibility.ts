...
import { expectNewWorkspaceDraft, openNewWorkspaceComposer } from "./new-workspace";
...
export const RECOVERY_PROMPT = "Keep this prompt while I fix the selection";
...
interface HiddenModelsDraftOptions {
  ...
  catalogClient: DaemonClient;
}

interface HiddenModelsDraft {
  page: Page;
  client: SeededWorkspace["client"];
  before: Awaited<ReturnType<typeof createdRecords>>;
}

interface HiddenModelRecoveryOptions {
  page: Page;
  catalogClient: DaemonClient;
  recovery: "restore" | "profile";
}

async function createdRecords(client: SeededWorkspace["client"]) {
  const workspaces = (await client.fetchWorkspaces()).entries;
  const agents = (await client.fetchAgents()).entries;
  return { workspaces, agents };
}

export async function openHiddenModelsDraft({
  page,
  workspace,
  catalogClient,
}: HiddenModelsDraftOptions): Promise<HiddenModelsDraft> {
  ...
  const before = await createdRecords(workspace.client);
  return { page, client: workspace.client, before };
}

async function expectUnsubmittedDraft({ page, client, before }: HiddenModelsDraft) {
  await expectNewWorkspaceDraft(page, RECOVERY_PROMPT);
  expect(await createdRecords(client)).toEqual(before);
}

export async function expectHiddenModelsRejected(draft: HiddenModelsDraft) {
  await expect(draft.page.getByTestId("new-workspace-submit-error")).toHaveText(
    "Every model for this provider is hidden. Show one in provider settings.",
  );
  await expectUnsubmittedDraft(draft);
}

export async function recoverHiddenModelSelection({
  page,
  catalogClient,
  recovery,
}: HiddenModelRecoveryOptions) {
  if (recovery === "restore") {
    await catalogClient.patchDaemonConfig({
...
export async function expectHiddenModelsRecovered(draft: HiddenModelsDraft) {
  await expectComposerModel(draft.page, HIDDEN_MODEL.label);
  await expect(draft.page.getByTestId("new-workspace-submit-error")).toHaveCount(0);
  await expectUnsubmittedDraft(draft);
}
