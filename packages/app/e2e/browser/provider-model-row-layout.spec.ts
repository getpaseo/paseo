...
interface RowLayoutDaemonClient {
...
async function seedProviderWithCustomModels() {
  const client = await connectDaemonClient<RowLayoutDaemonClient>({
    clientIdPrefix: "row-layout-e2e",
  });
  await client.patchDaemonConfig({
    providers: {
      [PROVIDER.id]: {
        ...
        description: `${PROVIDER.label} test provider`,
        enabled: true,
        command: ["node"],
        models: PROVIDER.models,
        additionalModels: PROVIDER.additionalModels,
      },
    },
  });
  return {
    async restore() {
      await client.patchDaemonConfig({ removeProviders: [PROVIDER.id] }).catch(() => undefined);
...
for (const compact of [false, true]) {
  test(`${compact ? "compact" : "desktop"} model rows keep controls aligned and text bounded`, async ({
    ...
    const seed = await seedProviderWithCustomModels();
    const workspace = await seedWorkspace({ repoPrefix: "model-row-layout-" });
    ...
    try {
      ...
      await test.step("every switch is inside the sheet and shares one right edge", async () => {
        ...
        for (let index = 0; index < count; index += 1) {
          ...
          expect(rect.x + rect.width, `${label} switch right edge`).toBeLessThanOrEqual(
            containerRight,
          );
