...
test("plugin header and composer buttons share actions, menus, content, and updates", async ({
  ...
  try {
    ...
    await test.step("wide buttons have placement-owned labels and chevrons", async () => {
      await expect(
        page
    ...
    await test.step("actions fail visibly, retry, and remain busy until completion", async () => {
      ...
      await expect(page.getByRole("button", { name: "Run review", exact: true })).toBeDisabled();
    ...
    await test.step("compact triggers and overflow open sheets with working plugin context", async () => {
    ...
    await test.step("hiding an open button and unloading a plugin remove its surfaces", async () => {
      await page.setViewportSize(WIDE);
      await press(page, "Composer checks");
      await showcase.setTitle("Hide composer menu");
      await expect(page.getByRole("menuitem", { name: "Run checks", exact: true })).toHaveCount(0);
      await expect(page.getByRole("button", { name: "Composer checks", exact: true })).toHaveCount(
        0,
      );
      await showcase.setTitle("Button showcase");
