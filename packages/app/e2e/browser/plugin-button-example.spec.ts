...
test("the button author example shows each capability without a competing overflow button", async ({
  ...
  try {
    ...
    await test.step("icon-only header action and labeled composer action update in place", async () => {
    ...
    await test.step("named menus include separators, nested actions, disabled items, and custom content", async () => {
      ...
      await expect(page.getByRole("button", { name: "Composer tools", exact: true })).toHaveCount(
        0,
      );
    ...
    await test.step("custom icons and content react to workspace changes and adapt to sheets", async () => {
      ...
      await expect(headerButtons(page)).toHaveCount(0);
      await expect(page.getByRole("button", { name: "Context details", exact: true })).toHaveCount(
        0,
      );
    });
  } finally {
