...
function SchedulesScreenBody({
  ...
  if (bodyState.kind === "load-error") {
    return (
      <View style={styles.centered}>
        <Text style={styles.message}>{t("schedules.errors.load")}</Text>
        <Button variant="ghost" onPress={onRetry} testID="schedules-retry">
          {t("schedules.actions.retry")}
        </Button>
      </View>
    );
  }
  ...
  return (
    <View style={styles.emptyState} testID={testID}>
      ...
      <View style={styles.emptyTextStack}>
        <Text style={styles.emptyTitle}>{t("schedules.empty.activeTitle")}</Text>
        <Text style={styles.emptyDescription}>{t("schedules.empty.activeDescription")}</Text>
        <ExternalLink href="https://paseo.sh/docs/schedules" label={t("schedules.actions.docs")} />
      </View>
  ...
  return (
    <View style={styles.filterEmpty}>
      <View style={styles.endedEmptyState}>
        <CalendarClock size={styles.emptyIcon.width} color={styles.emptyIcon.color} />
        <Text style={styles.emptyTitle}>{t("schedules.empty.endedTitle")}</Text>
      </View>
    </View>
  );
}
      ...
      <View style={styles.errorsBanner} testID="schedules-host-errors">
        {errors.map((error) => (
          <Text key={error.serverId} style={styles.errorsBannerText}>
            {t("schedules.errors.hostLoad", { hostName: error.serverName })}
          </Text>
        ))}
      </View>
    </View>
