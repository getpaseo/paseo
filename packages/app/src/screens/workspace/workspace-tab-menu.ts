...
function getCloseButtonTestId(tab: WorkspaceTabDescriptor): string {
  ...
  if (tab.target.kind === "chapter" || tab.target.kind === "chapters")
    return `workspace-chapter-close-${tab.tabId}`;
...
export function buildWorkspaceTabMenuEntries(
  ...
  if (tab.target.kind === "agent" || tab.target.kind === "terminal") {
    ...
    if (tab.target.kind === "agent" && onUseAutomaticNaming) {
      entries.push({
        kind: "item",
        key: "automatic-naming",
        label: i18n.t("responseControl.automaticNaming"),
        icon: "rotate-cw",
        testID: `${menuTestIDBase}-automatic-naming`,
        onSelect: () => onUseAutomaticNaming(tab),
      });
    }
    entries.push({
      kind: "separator",
...
export function buildWorkspaceDesktopTabActions(
  ...
  return {
    ...
    menuEntries: buildWorkspaceTabMenuEntries({
      ...
      onUseAutomaticNaming: input.onUseAutomaticNaming,
