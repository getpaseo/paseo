...
describe("SuggestionModelSection", () => {
  ...
  it("saves the picked model as the suggestion model", async () => {
    render(metadata());
    ...
    expect(patchConfig).toHaveBeenCalledWith({
      metadataGeneration: { promptSuggestions: { providers: entries } },
    });
  });

  it("empties the suggestion list when switched back to automatic", async () => {
    const entries = [{ provider: "codex-auto", model: "gpt-6-astra" }];
