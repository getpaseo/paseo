...
describe("processTimelineResponse", () => {
  ...
  it("replaces the bounded timeline window for a prompt jump", () => {
    const result = processTimelineResponse({
      ...baseTimelineInput,
      currentTail: [
        {
    ...
    expect(getUserTexts(result.tail)).toEqual(["oldest", "pending submission"]);
    expect(result.cursor).toEqual({ epoch: "epoch-1", startSeq: 1, endSeq: 40 });
