...
describe("loadAppSettingsFromStorage", () => {
  ...
  it("defaults send button visibility to always when storage is empty", async () => {
    const deps = makeDeps();

    const result = await loadAppSettingsFromStorage(deps);

    expect(result.sendButtonVisibility).toBe("always");
  });

  it("loads send button visibility from app settings", async () => {
    const deps = makeDeps({
      storage: createInMemoryKeyValueStorage({
        [APP_SETTINGS_KEY]: JSON.stringify({ sendButtonVisibility: "whenContent" }),
      }),
    });

    const result = await loadAppSettingsFromStorage(deps);

    expect(result.sendButtonVisibility).toBe("whenContent");
  });

  it("defaults enter key behavior to send when storage is empty", async () => {
    const deps = makeDeps();
  ...
  it("loads enter key behavior from app settings", async () => {
    const deps = makeDeps({
      storage: createInMemoryKeyValueStorage({
        [APP_SETTINGS_KEY]: JSON.stringify({ enterKeyBehavior: "newline" }),
      }),
    });

    const result = await loadAppSettingsFromStorage(deps);

    expect(result.enterKeyBehavior).toBe("newline");
  });

  it("falls back to send for unknown enter key behavior values", async () => {
    const deps = makeDeps({
      storage: createInMemoryKeyValueStorage({
        [APP_SETTINGS_KEY]: JSON.stringify({ enterKeyBehavior: "submit-and-newline" }),
      }),
    });
