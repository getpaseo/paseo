  ...
  settings: {
    ...
    host: {
      ...
      orchestration: {
        ...
        managedMcp: {
          title: "外部 MCP サーバー",
          hint: "このホストが起動する互換エージェントすべてに共有サーバーを注入します。",
          empty: "外部 MCP サーバーは設定されていません。",
          addServer: "サーバーを追加",
          ...
          url: "URL",
          arguments: "引数",
          argumentsHint: "1 行に 1 つの引数を入力します。",
          alwaysLoad: "常にツールを読み込む",
          alwaysLoadHint: "このサーバーのすべてのツールをエージェントのプロンプトに保持します。",
          alwaysLoadAccessibility: "MCP ツールを常に読み込む",
          environmentVariables: "環境変数",
          requestHeaders: "リクエストヘッダー",
          secretHint: "非公開の直接値を使用するか、daemon の環境を参照します。",
          add: "追加",
          directValue: "直接値",
          environmentSource: "環境",
          variable: "変数",
          header: "ヘッダー",
          environmentVariableName: "環境変数名",
          value: "値",
          keepValueHint: "設定済みの値を保持するには空欄にします。",
          save: "保存",
          test: "テスト",
          edit: "編集",
          remove: "削除",
          enable: "{{name}} を有効化",
          connected: "{{latencyMs}} ms で接続 · {{toolCount}} ツール",
          rejected: "MCP サーバーが接続を拒否しました。",
          duplicateName: "'{{name}}' という MCP サーバーは既に存在します。",
          updateFailed: "MCP サーバーを更新できません。",
          removeTitle: "MCP サーバーを削除しますか？",
          removeMessage: "このホストが起動するすべてのエージェントから '{{name}}' を削除しますか？",
          removeFailed: "MCP サーバーを削除できません。",
          testFailed: "MCP サーバーをテストできません。",
          saveFailed: "MCP サーバーを保存できません。",
          entryNameRequired: "各ヘッダーまたは環境エントリには名前が必要です。",
          envNameRequired: "環境参照には変数名が必要です。",
          directValueRequired: "直接値を空にすることはできません。",
          uniqueEntries: "ヘッダー名と環境エントリ名は一意である必要があります。",
          nameRequired: "サーバー名は必須です。",
          commandRequired: "コマンドは必須です。",
          urlRequired: "URL は必須です。",
          invalidUrl: "有効な MCP サーバー URL を入力してください。",
          httpOnly: "MCP サーバー URL は HTTP または HTTPS を使用する必要があります。",
        },
        systemPrompt: {
          title: "システムプロンプト",
