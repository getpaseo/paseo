...
import { buildGitActions, buildPrPanelMergeActions, type BuildGitActionsInput } from "./policy";
...
describe("buildPrPanelMergeActions", () => {
  function mergeableInput(overrides: Partial<BuildGitActionsInput> = {}): BuildGitActionsInput {
    return createInput({
      hasRemote: true,
      isOnBaseBranch: false,
      aheadCount: 2,
  ...
  it("builds a mergeable split: preferred method primary, other allowed methods secondary", () => {
    const actions = buildPrPanelMergeActions(
      mergeableInput({
        pullRequestGithub: githubStatus({
          repository: {
            autoMergeAllowed: true,
  ...
  it("honors viewerDefaultMergeMethod for the primary merge method", () => {
    const actions = buildPrPanelMergeActions(
      mergeableInput({
        pullRequestGithub: githubStatus({
          repository: {
    ...
    expect(actions.primary?.id).toBe("merge-pr-rebase");
  });
  ...
  it.each([
    ...
    "builds a muted primary with the reason and empty secondary when the PR is %s",
    (_name, overrides, expectedMessage) => {
      const actions = buildPrPanelMergeActions(mergeableInput(overrides));

      expect(actions.primary).toMatchObject({
        id: "merge-pr-squash",
        disabled: false,
        unavailableMessage: expectedMessage,
      });
      expect(actions.secondary).toEqual([]);
      expect(actions.menu).toEqual([]);
    },
  ...
  it("builds a muted primary with the not-ready reason when GitHub checks block the merge", () => {
    const actions = buildPrPanelMergeActions(
      mergeableInput({
        githubAutoMergeActionsEnabled: false,
  ...
  it("offers enable-auto-merge as primary when the PR is auto-merge eligible", () => {
    const actions = buildPrPanelMergeActions(
      mergeableInput({
        pullRequestGithub: githubStatus({
          mergeStateStatus: "BLOCKED",
  ...
  it("offers disable-auto-merge as primary when auto-merge is already enabled", () => {
    const actions = buildPrPanelMergeActions(
      mergeableInput({
        pullRequestGithub: githubStatus({
          mergeStateStatus: "BLOCKED",
  ...
  it.each([
    ["merged", { pullRequestIsMerged: true }],
    ["closed", { pullRequestState: "closed" as const }],
  ])("returns empty actions when the PR is %s", (_name, overrides) => {
    const actions = buildPrPanelMergeActions(mergeableInput(overrides));

    expect(actions).toEqual({ primary: null, secondary: [], menu: [] });
  });

  it("returns empty actions when there is no pull request", () => {
    const actions = buildPrPanelMergeActions(createInput({ isOnBaseBranch: false, aheadCount: 2 }));
