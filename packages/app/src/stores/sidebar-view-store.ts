...
export type SidebarGroupMode = "project" | "status" | "recentActivity";
...
const PersistedSidebarGroupModeSchema = z.enum(["project", "status", "label", "recentActivity"]);
...
export function migrateSidebarViewState(persistedState: unknown): SidebarViewPersistedState {
  ...
  return {
    groupMode:
      state.groupMode === "status" || state.groupMode === "recentActivity"
        ? state.groupMode
        : "project",
