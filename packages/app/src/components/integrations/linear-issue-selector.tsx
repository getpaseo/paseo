// LinearIssueSelector — adapted from emdash for React Native
// Search and select Linear issues for task context
...
interface LinearIssueSelectorProps {
  ...
  onSelect: (issue: LinearIssueSummary | null) => void;
...
export function LinearIssueSelector({
  issues = [],
  selectedIssueId,
  onSelect,
  onSearch,
  ...
  if (selectedIssue) {
    return (
      <View style={selectorStyles.selectedContainer}>
        <View style={selectorStyles.selectedContent}>
          <Text style={selectorStyles.selectedIdentifier}>{selectedIssue.identifier}</Text>
          <Text style={selectorStyles.selectedTitle} numberOfLines={1}>
            {selectedIssue.title}
          </Text>
        </View>
        <Pressable onPress={handleClear} style={selectorStyles.clearButton}>
        ...
        <View style={selectorStyles.dropdown}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                style={({ pressed }) => [
                  selectorStyles.issueRow,
                  pressed && selectorStyles.issueRowPressed,
                ]}
                onPress={() => handleSelect(item)}
              >
                <Text style={selectorStyles.issueIdentifier}>{item.identifier}</Text>
                <Text style={selectorStyles.issueTitle} numberOfLines={1}>
                  {item.title}
                </Text>
              </Pressable>
            )}
            style={selectorStyles.dropdownList}
          />
        </View>
      ) : null}
    </View>
