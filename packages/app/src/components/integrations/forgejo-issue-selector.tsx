// ForgejoIssueSelector — adapted from emdash for React Native
...
interface ForgejoIssueSelectorProps {
  issues?: ForgejoIssueSummary[];
  selectedIssueId?: number | null;
  onSelect: (issue: ForgejoIssueSummary | null) => void;
  onSearch?: (query: string) => void;
  isLoading?: boolean;
  placeholder?: string;
}

export function ForgejoIssueSelector({
  issues = [],
  selectedIssueId,
  onSelect,
  onSearch,
  isLoading = false,
  placeholder = "Search Forgejo issues...",
}: ForgejoIssueSelectorProps) {
  ...
  if (selected) {
    return (
      <View style={fStyles.selectedContainer}>
        <Text style={fStyles.selectedId}>#{selected.number}</Text>
        <Text style={fStyles.selectedTitle} numberOfLines={1}>
          {selected.title}
        </Text>
        <Pressable onPress={() => onSelect(null)} style={fStyles.clearBtn}>
      ...
      {isOpen && issues.length > 0 ? (
        <View style={fStyles.dropdown}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                style={({ pressed }) => [fStyles.row, pressed && fStyles.rowPressed]}
                onPress={() => {
                  onSelect(item);
                  setIsOpen(false);
                }}
              >
                <Text style={fStyles.rowId}>#{item.number}</Text>
                <Text style={fStyles.rowTitle} numberOfLines={1}>
                  {item.title}
                </Text>
              </Pressable>
            )}
...
const fStyles = StyleSheet.create((theme) => ({
