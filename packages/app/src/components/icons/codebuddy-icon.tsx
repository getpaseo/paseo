...
interface CodeBuddyIconProps {
  size?: number;
  // Unused for the CodeBuddy icon (brand colors are part of the mark),
  // accepted for API parity with the other provider icons.
  color?: string;
}

// Official CodeBuddy mark from @tencent-ai/codebuddy-code branding
// (source: AionUi assets/codebuddy.svg, identical in both bundled copies).
// The decorative Gaussian-blur circles from the source SVG are dropped because
// react-native-svg's filter support is uneven across platforms; the gradient
// background, white "P" path, and the two cursor-mark rectangles are
// preserved.
export function CodeBuddyIcon({ size = 16 }: CodeBuddyIconProps) {
  return (
    <Svg width={size} height={size} viewBox="0 0 40 40">
      <Defs>
        <ClipPath id="codebuddy-clip">
      ...
      <Rect width="40" height="40" rx="20" fill="url(#codebuddy-bg)" />
      <Path
        clipPath="url(#codebuddy-clip)"
        fillRule="evenodd"
        fill="#FFFFFF"
