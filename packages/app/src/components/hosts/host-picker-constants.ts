...
export interface HostPickerSystemLabels {
  allHosts?: string;
  addHost?: string;
  enableBuiltInDaemon?: string;
  hostFallback?: string;
}
...
export function getHostPickerLabel(
  ...
  config?: {
    ...
    labels?: HostPickerSystemLabels;
  },
): string {
  const labels = config?.labels;
  if (config?.includeAllHost && value === ALL_HOSTS_OPTION_ID) {
    return labels?.allHosts ?? "All hosts";
  }
  if (config?.includeAddHost && value === ADD_HOST_OPTION_ID) {
    return labels?.addHost ?? "Add host";
  }
  return (
    hosts.find((host) => host.serverId === value)?.label ??
    (config?.includeAllHost ? (labels?.allHosts ?? "All hosts") : (labels?.hostFallback ?? "Host"))
  );
}
