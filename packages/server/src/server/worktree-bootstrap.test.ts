...
describe("runAsyncWorktreeBootstrap", () => {
  ...
  // POSIX-only: setup command fixtures use POSIX shell syntax.
  it.skipIf(isPlatform("win32"))(
    "streams running setup updates live and persists only a final setup timeline row",
    async () => {
      writeFileSync(
  ...
  // POSIX-only: command log fixture depends on POSIX shell command quoting.
  it.skipIf(isPlatform("win32"))(
    "keeps only the final carriage-return-updated content in command logs",
    async () => {
      writeFileSync(
        join(repoDir, "paseo.json"),
        JSON.stringify({
  ...
  // POSIX-only: setup fixture writes files through POSIX shell commands.
  it.skipIf(isPlatform("win32"))(
    "shares the same worktree runtime port across setup and bootstrap terminals",
    async () => {
      writeFileSync(
        join(repoDir, "paseo.json"),
        JSON.stringify({
  ...
  // POSIX-only: terminal-backed service fixture uses POSIX shell commands.
  it.skipIf(isPlatform("win32"))(
    "injects real peer service env into terminal-backed services",
    async () => {
      writeFileSync(
        join(repoDir, "paseo.json"),
        JSON.stringify({
