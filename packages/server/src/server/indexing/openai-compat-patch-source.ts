// Auto-bundled from `docs/patches/crg-openai-compat/embeddings_openai_compat.py`.
// Injected into the installed crg venv by `installer.ts` -> `injectOpenAICompatPatch`.
// When you edit the Python source, re-run the bundle step.

export const OPENAI_COMPAT_PROVIDER_SOURCE = `"""OpenAI-compatible embedding provider for code-review-graph.
