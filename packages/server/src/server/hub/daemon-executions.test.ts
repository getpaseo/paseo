...
test("Hub execution accepts a long prompt without a worktree", async () => {
  const hub = await launchRelationship();
...
test("Hub execution accepts a long prompt with a branch-off worktree", async () => {
  const hub = await launchRelationship();
  const prompt = `Implement the issue.\n\n${"x".repeat(1_000)}`;

  hub.beginOwnedCreate("long-prompt-worktree", "long-prompt-worktree-execution", {
    prompt,
    worktree: {
      mode: "branch-off",
  ...
  const response = await hub.ownedCreateResult("long-prompt-worktree");

  expect(response).toMatchObject({
    type: "hub.execution.agent.create.response",
    payload: {
      success: true,
      executionId: "long-prompt-worktree-execution",
      error: null,
    },
  });
