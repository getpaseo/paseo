...
describe("daemon bearer validator", () => {
  ...
  test("extracts WebSocket hubcode bearer subprotocol tokens", () => {
    const protocol = extractWsBearerProtocol("chat, hubcode.bearer.secret.with.dots");

    expect(protocol).toBe("hubcode.bearer.secret.with.dots");
    expect(extractWsBearerToken(protocol)).toBe("secret.with.dots");
    expect(extractWsBearerToken("hubcode.other.secret")).toBeNull();
  });
});
