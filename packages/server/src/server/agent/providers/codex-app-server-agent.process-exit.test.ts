...
test("provider exit and reconnect keep terminal events on their owning turns", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "codex-process-turn-ownership-"));
  const exitedAppServer = createFakeCodexAppServer();
  const replacementAppServer = createFakeCodexAppServer();
  const appServers = [exitedAppServer, replacementAppServer];
  const session = new CodexAppServerAgentSession(
    { provider: "codex", cwd: workdir, modeId: "auto", model: "gpt-5.4" },
    null,
    logger,
  ...
  try {
    await session.connect();
    const firstTurn = await session.startTurn("run before provider exit");
    const firstTurnStart = await exitedAppServer.waitForTurnStart();
    exitedAppServer.startsTurn({
    ...
    await expect
      .poll(() => events.filter((event) => event.type === "turn_failed"))
      .toEqual([
        {
          type: "turn_failed",
          provider: "codex",
    ...
    const secondTurn = await session.startTurn("continue after reconnect");
    const secondTurnStart = await replacementAppServer.waitForTurnStart();
    const secondThreadId = String(secondTurnStart.threadId);
    replacementAppServer.startsTurn({ threadId: secondThreadId, turnId: "native-turn-2" });
    replacementAppServer.completeTurn({ threadId: secondThreadId, turnId: "native-turn-2" });

    await expect
      .poll(() =>
        events.filter((event) => event.type === "turn_failed" || event.type === "turn_completed"),
      )
      .toEqual([
        {
          type: "turn_failed",
