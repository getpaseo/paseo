  ...
  test("custom agents defined in opencode.json appear in available modes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "hubcode-test-custom": {
            description: "Custom agent defined for Hubcode integration test",
            mode: "primary",
          },
        },
      }),
    ...
    const custom = modes.find((mode) => mode.id === "hubcode-test-custom");
    expect(custom).toBeDefined();
    expect(custom!.label).toBe("Hubcode-test-custom");
    expect(custom!.description).toBe("Custom agent defined for Hubcode integration test");
