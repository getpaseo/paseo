...
interface PersistedAssistantCacheTokens {
  read?: number;
  write?: number;
}

interface PersistedAssistantTokens {
  input?: number;
  output?: number;
  reasoning?: number;
  cache?: PersistedAssistantCacheTokens;
}
...
function persistedAssistantMessage(params: PersistedAssistantMessageParams): unknown {
  return {
    info: {
      id: params.id,
      ...
      ...(params.structured !== undefined ? { structured: params.structured } : {}),
...
async function* sessionIdleStream(releaseIdle: ReturnType<typeof createTestDeferred<void>>) {
...
async function createControlledEofTurn(params?: { baselineMessages?: unknown[] }): Promise<{
  parent: Awaited<ReturnType<OpenCodeAgentClient["createSession"]>>;
  openCode: TestOpenCodeClient;
  events: AgentStreamEvent[];
  endStream: (events?: unknown[], beforeOwnershipEvents?: unknown[]) => void;
  ...
  const { parent, openCode } = await createParentSession("ses_eof_reconciliation", (client) => {
    ...
    client.sessionMessagesResponse = { data: params?.baselineMessages ?? [] };
    client.globalEventImplementation = async (options) => {
      const signal = (options as { signal: AbortSignal }).signal;
      return {
        stream: (async function* () {
          yield { type: "server.connected", properties: {} };
          const finalEvents = await Promise.race([
            streamEnd.promise,
  ...
  return {
    ...
    openCode,
    events,
    endStream: (finalEvents = [], beforeOwnershipEvents = []) =>
      streamEnd.resolve([
        ...beforeOwnershipEvents,
        {
          type: "message.updated",
          properties: {
            info: {
...
async function createControlledAutonomousEofTurn(): Promise<{
  parent: Awaited<ReturnType<OpenCodeAgentClient["createSession"]>>;
  openCode: TestOpenCodeClient;
  events: AgentStreamEvent[];
  endStream: () => void;
  ...
  const initiatingMessageId = "msg_autonomous_initiating";
  const releaseAutonomousTurn = createTestDeferred<void>();
  const streamEnd = createTestDeferred<void>();
  const { parent, openCode } = await createParentSession("ses_eof_reconciliation", (client) => {
    client.globalEventImplementation = async (options) => {
      const signal = (options as { signal: AbortSignal }).signal;
      return {
        stream: (async function* () {
          yield { type: "server.connected", properties: {} };
          const shouldStart = await Promise.race([
            releaseAutonomousTurn.promise.then(() => true),
...
describe("OpenCode adapter startTurn error handling", () => {
  ...
  test.each([
    {
      label: "completed",
      message: { id: "msg_completed", completed: 4 },
  ...
  ])("reconciles a persisted $label turn when the event stream ends", async (testCase) => {
    ...
    openCode.sessionMessagesResponse = { data: [initiatingMessage, assistantMessage] };

    try {
      endStream();
      await vi.waitFor(() => {
        expect(terminalTurnEvents(events)).toEqual([expect.objectContaining(testCase.expected)]);
      });
  ...
  test("uses the pre-dispatch baseline to reject stale foreground ownership before EOF", async () => {
    const staleUser = persistedUserMessage("msg_stale_user");
    const staleAssistant = persistedAssistantMessage({
      id: "msg_stale_assistant",
      parentId: "msg_stale_user",
      completed: 2,
      parts: [
        {
          id: "prt_stale_answer",
          sessionID: "ses_eof_reconciliation",
          messageID: "msg_stale_assistant",
          type: "text",
          text: "Stale answer.",
        },
      ],
    ...
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledEofTurn({ baselineMessages: [staleUser, staleAssistant] });
    openCode.sessionMessagesResponse = {
      data: [
        staleUser,
        staleAssistant,
        ...
        [
          {
            type: "message.updated",
            properties: {
              info: { id: "msg_stale_user", sessionID: "ses_eof_reconciliation", role: "user" },
            },
          },
          {
            type: "message.updated",
  ...
  test("replays structured-only assistant output exactly once before EOF completion", async () => {
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledEofTurn();
    openCode.sessionMessagesResponse = {
      data: [
      ...
      expect(
        events.filter(
          (event) =>
            ...
            event.item.text === '{"result":"recovered"}',
        ),
      ).toHaveLength(1);
    } finally {
      await parent.close();
  ...
  test("suppresses persisted compact summary text when a compact stream ends at EOF", async () => {
    const { parent, openCode, events, endStream, baselineMessages } =
      await createControlledCommandEofTurn({ prompt: "/compact" });
    openCode.sessionMessagesResponse = {
      data: [
        ...baselineMessages,
        persistedUserMessage("msg_compact_user"),
        persistedAssistantMessage({
          id: "msg_compact_summary",
      ...
      expect(
        events.some(
          (event) =>
            event.type === "timeline" &&
            event.item.type === "assistant_message" &&
            event.item.text === "Internal compact summary.",
        ),
      ).toBe(false);
    } finally {
      await parent.close();
  ...
  test("suppresses recognized compaction summary messages during EOF replay", async () => {
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledEofTurn();
    openCode.sessionMessagesResponse = {
      data: [
        initiatingMessage,
        persistedAssistantMessage({
          id: "msg_recognized_compaction_summary",
          parentId: initiatingMessageId,
          completed: 4,
          summary: true,
          agent: "compaction",
          ...
          parts: [
            {
              id: "prt_recognized_compaction_summary",
              sessionID: "ses_eof_reconciliation",
              messageID: "msg_recognized_compaction_summary",
              type: "text",
              text: "Recognized compact summary.",
            },
          ],
        }),
      ],
      ...
      expect(
        events.some(
          (event) =>
            event.type === "timeline" &&
            event.item.type === "assistant_message" &&
            event.item.text === "Recognized compact summary.",
        ),
      ).toBe(false);
    } finally {
      await parent.close();
  ...
  test.each([
    ...
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledEofTurn();
    const messages = testCase.includeAssistant
      ? [
          initiatingMessage,
          persistedAssistantMessage({
            id: "msg_incomplete",
    ...
    openCode.sessionMessagesResponse = { data: messages };

    try {
      endStream();
      await vi.waitFor(() => {
        expect(eventsWithType(events, "turn_failed")).toEqual([
          expect.objectContaining({
            type: "turn_failed",
  ...
  test("ignores an interleaved unrelated assistant error after the owned completion", async () => {
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledEofTurn();
    openCode.sessionMessagesResponse = {
      data: [
        initiatingMessage,
        persistedAssistantMessage({
          id: "msg_owned_completed",
          parentId: initiatingMessageId,
          completed: 4,
        }),
        persistedUserMessage("msg_unrelated_user"),
        persistedAssistantMessage({
          id: "msg_unrelated_failed",
          parentId: "msg_unrelated_user",
          error: { name: "UnknownError", data: { message: "unrelated failure" } },
        }),
      ],
    };
  ...
  test("ignores an interleaved unrelated completion after the owned assistant error", async () => {
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledEofTurn();
    openCode.sessionMessagesResponse = {
      data: [
        ...
        persistedAssistantMessage({
          id: "msg_owned_failed",
          parentId: initiatingMessageId,
          error: { name: "UnknownError", data: { message: "owned failure" } },
        }),
        persistedUserMessage("msg_unrelated_user"),
        persistedAssistantMessage({
          id: "msg_unrelated_completed",
          parentId: "msg_unrelated_user",
          completed: 5,
        }),
      ],
  ...
  test("normalizes and merges persisted assistant usage before EOF completion", async () => {
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledEofTurn();
    openCode.sessionMessagesResponse = {
      data: [
      ...
      endStream([
        {
          ...
          properties: {
            part: {
              id: "prt_streamed_usage",
              sessionID: "ses_eof_reconciliation",
              messageID: "msg_usage",
              type: "step-finish",
              cost: 0.1,
              tokens: { input: 2, output: 1, cache: { read: 1, write: 1 } },
            },
          },
        },
      ]);
  ...
  test.each([
    {
      label: "completed",
      message: { id: "msg_autonomous_completed", completed: 4 },
      expected: { type: "turn_completed", provider: "opencode" },
    },
    {
      label: "failed",
      message: {
        id: "msg_autonomous_failed",
        error: { name: "UnknownError", data: { message: "autonomous failure" } },
      },
      expected: {
        type: "turn_failed",
        provider: "opencode",
    ...
    },
    {
      label: "interrupted",
      message: {
        id: "msg_autonomous_interrupted",
        error: { name: "MessageAbortedError", data: { message: "aborted" } },
      },
      expected: { type: "turn_canceled", provider: "opencode", reason: "interrupted" },
    },
  ])("reconciles an owned autonomous $label turn after EOF", async (testCase) => {
    const { parent, openCode, events, endStream, initiatingMessageId, initiatingMessage } =
      await createControlledAutonomousEofTurn();
    openCode.sessionMessagesResponse = {
      data: [
        initiatingMessage,
        persistedAssistantMessage({
  ...
  test.each([
    ...
    openCode.globalEventImplementation = async () => ({
      stream: (async function* () {
        ...
        if (testCase.label === "permission-first") {
          yield {
            type: "session.status",
            properties: {
              sessionID: "ses_late_owned_autonomous",
  ...
  test("establishes autonomous EOF ownership only after the busy boundary", async () => {
    const releaseEvents = createTestDeferred<void>();
    const staleUser = persistedUserMessage("msg_stale_autonomous_user", "ses_boundary_owned");
    const staleAssistant = persistedAssistantMessage({
      id: "msg_stale_autonomous_assistant",
      parentId: "msg_stale_autonomous_user",
      sessionId: "ses_boundary_owned",
      completed: 2,
      parts: [
        {
          id: "prt_stale_autonomous",
          sessionID: "ses_boundary_owned",
          messageID: "msg_stale_autonomous_assistant",
          type: "text",
          text: "Stale autonomous answer.",
        },
      ],
    });
    const currentUser = persistedUserMessage("msg_current_autonomous_user", "ses_boundary_owned");
    const currentAssistant = persistedAssistantMessage({
      id: "msg_current_autonomous_assistant",
      parentId: "msg_current_autonomous_user",
      sessionId: "ses_boundary_owned",
      completed: 4,
      parts: [
        {
          id: "prt_current_autonomous",
          sessionID: "ses_boundary_owned",
          messageID: "msg_current_autonomous_assistant",
          type: "text",
          text: "Current autonomous answer.",
        },
      ],
    });
    const openCode = new TestOpenCodeClient();
    openCode.sessionMessagesResponse = {
      data: [staleUser, staleAssistant, currentUser, currentAssistant],
    };
    openCode.globalEventImplementation = async () => ({
      stream: (async function* () {
        yield { type: "server.connected", properties: {} };
        ...
        yield {
          type: "session.status",
          properties: { sessionID: "ses_boundary_owned", status: { type: "busy" } },
        };
        yield {
          type: "message.updated",
          properties: {
            info: {
              id: "msg_current_autonomous_user",
              sessionID: "ses_boundary_owned",
              role: "user",
            },
          },
    ...
    try {
      ...
      ).toEqual(["Current autonomous answer."]);
    } finally {
      releaseEvents.resolve();
      await session.close();
    }
  ...
  test("does not duplicate a terminal event when the event stream ends after session idle", async () => {
    ...
    const { parent, openCode } = await createParentSession("ses_eof_reconciliation", (client) => {
      ...
      client.globalEventImplementation = async () => ({
        stream: sessionIdleStream(releaseIdle),
      });
    });
    const events: AgentStreamEvent[] = [];
    parent.subscribe((event) => events.push(event));
