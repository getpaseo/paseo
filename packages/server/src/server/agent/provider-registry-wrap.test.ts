...
describe("wrapClientProvider", () => {
  test("forwards provider-health cancellation to the inner provider", async () => {
    const isAvailable = vi.fn(async () => true);
    const wrapped = wrapClientProvider(
      "custom-opencode",
      createFakeClient({ isAvailable }),
      [],
      [],
      false,
    );
    ...
    await expect(wrapped.isAvailable({ signal: controller.signal })).resolves.toBe(true);
    expect(isAvailable).toHaveBeenCalledWith({ signal: controller.signal });
  });
