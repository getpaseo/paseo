...
test("reloadAgentSession preserves running provider children by default", async () => {
  ...
  try {
    ...
    activeSession?.pushEvent({
      type: "provider_subagent",
      provider: "codex",
      event: { type: "upsert", id: "running-child", status: "running" },
    });
    await manager.flush();

    await manager.reloadAgentSession(parent.id);

    expect(manager.listProviderSubagents(parent.id)).toEqual([
      expect.objectContaining({ id: "running-child", status: "running" }),
    ]);
  } finally {
...
test("collectIdleAgents keeps idle ancestors resident while a managed descendant is running", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-running-descendant-"));
  const client = new (class extends TestAgentClient {
  ...
  try {
    ...
    client.sessions[2]!.pushEvent({
      type: "turn_started",
      provider: "codex",
      turnId: "running-grandchild",
    });
    await manager.flush();
    ...
    expect(collection).toEqual({ collected: [], failures: [] });
    expect(manager.getAgent(root.id)?.lifecycle).toBe("idle");
...
test("collectIdleAgents protects ancestors through a collected intermediate agent", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-collected-intermediate-"));
  const client = new (class extends TestAgentClient {
    readonly sessions: TestAgentSession[] = [];
...
test("collectIdleAgents refreshes descendant state after an earlier close", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-collection-state-race-"));
    ...
    override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
      const session =
        this.sessions.length === 0
          ? new (class extends TestAgentSession {
              override async close(): Promise<void> {
  ...
  try {
    ...
    client.sessions[2]!.pushEvent({
      type: "turn_started",
      provider: "codex",
      turnId: "started-during-collection",
    });
    await manager.flush();
    closeAllowed.resolve();

    await expect(collection).resolves.toEqual({
      collected: [expect.objectContaining({ agentId: blocker.id })],
      failures: [],
    });
...
test("collectIdleAgents closes managed descendants before their ancestors", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-descendant-close-order-"));
  const closeOrder: string[] = [];
  const client = new (class extends TestAgentClient {
    private sessionCount = 0;

    override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
      const label = this.sessionCount++ === 0 ? "parent" : "child";
      return new (class extends TestAgentSession {
        override async close(): Promise<void> {
...
test("collectIdleAgents serializes descendant publication with ancestor collection", async () => {
  ...
  try {
    ...
    const childRegistration = manager.createAgent({ provider: "codex", cwd: workdir }, undefined, {
      labels: { [PARENT_AGENT_ID_LABEL]: parent.id },
      workspaceId: undefined,
    });
    ...
    closeAllowed.resolve();
    await expect(collection).resolves.toEqual({
      collected: [expect.objectContaining({ agentId: parent.id })],
      failures: [],
    });
...
test("collectIdleAgents keeps an idle agent resident while a provider subagent is running", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-running-provider-subagent-"));
  let session: TestAgentSession | null = null;
  const client = new (class extends TestAgentClient {
    override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
      session = new TestAgentSession(config);
  ...
  try {
    ...
    expect(collection).toEqual({ collected: [], failures: [] });
    expect(manager.listProviderSubagents(parent.id)).toEqual([
      expect.objectContaining({ id: "provider-child", status: "running" }),
    ]);
    expect(manager.getAgent(parent.id)?.lifecycle).toBe("idle");
    ...
    await manager.flush();
    const providerChildUpdatedAt = Date.parse(
      manager.listProviderSubagents(parent.id)[0]!.updatedAt,
    );
...
test("closing a runtime invalidates retained running provider subagents", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-close-provider-subagent-"));
  const storage = new AgentStorage(join(workdir, "agents"), logger);
  let session: TestAgentSession | null = null;
  const client = new (class extends TestAgentClient {
    override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
  ...
  try {
    ...
    expect(manager.listProviderSubagents(parent.id)).toEqual([
      expect.objectContaining({ id: "provider-child", status: "canceled" }),
    ]);
  } finally {
    await Promise.all(manager.listAgents().map((agent) => manager.closeAgent(agent.id))).catch(
      () => undefined,
...
test("collectIdleAgents starts an ancestor's idle window after managed descendant activity", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-descendant-idle-window-"));
  const client = new (class extends TestAgentClient {
    readonly sessions: TestAgentSession[] = [];
