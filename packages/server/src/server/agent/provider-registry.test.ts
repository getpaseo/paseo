...
vi.mock("./providers/dsh-acp-agent.js", () => ({
  DshACPAgentClient: class DshACPAgentClient {
    readonly capabilities = {
      supportsStreaming: true,
      supportsSessionPersistence: true,
      supportsDynamicModes: true,
...
test("dsh provider extending acp uses DshACPAgentClient", () => {
  const registry = buildProviderRegistry(logger, {
    providerOverrides: {
      dsh: {
        extends: "acp",
  ...
  expect(mockState.constructorArgs.dsh).toEqual([
    {
      command: ["dsh", "--profile", "acp"],
      env: undefined,
      providerParams: undefined,
