...
export class AgentManager {
  ...
  resumeAgentFromPersistence(
    ...
    return this.trackAgentRegistrationOperation(
      this.runLifecycleMutation(resolvedAgentId, () =>
        this.resumeAgentFromPersistenceInternal(handle, overrides, resolvedAgentId, options),
      ),
    );
  }
  ...
  private async resumeAgentFromPersistenceInternal(
    ...
    // have read the record before a queued archive completed; an archived agent
    // only gets the read-only history, never an interactive writer.
