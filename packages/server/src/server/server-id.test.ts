...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "hubcode-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.HUBCODE_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per HUBCODE_HOME", () => {
  ...
  it("respects and persists HUBCODE_SERVER_ID override", () => {
    process.env.HUBCODE_SERVER_ID = "test-daemon-id";
