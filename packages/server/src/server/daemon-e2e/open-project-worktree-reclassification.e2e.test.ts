...
const cleanupDaemons = new Set<TestHubcodeDaemon>();
...
test("openProject reclassifies an existing directory workspace into its parent git project", async () => {
  ...
  try {
    ...
    const hubcodeHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "hubcode-open-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(worktreeRoot);
    cleanupPaths.add(hubcodeHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@gethubcode.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'Hubcode Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    const projectsPath = path.join(hubcodeHome, "projects", "projects.json");
    const workspacesPath = path.join(hubcodeHome, "projects", "workspaces.json");
    ...
    const daemon = await createTestHubcodeDaemon({ hubcodeHomeRoot, cleanup: false });
    ...
    process.env.HUBCODE_SUPERVISED = previousSupervised;
  }
}, 30_000);
