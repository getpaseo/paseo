 ...
 *   HUBCODE_LISTEN=127.0.0.1:7777 npx tsx packages/server/src/server/daemon-e2e/checkout-debug.ts
 */
...
const HUBCODE_HOME = process.env.HUBCODE_HOME ?? `${os.homedir()}/.hubcode`;
const HUBCODE_LISTEN = process.env.HUBCODE_LISTEN ?? "127.0.0.1:6767";
const DAEMON_URL = `ws://${HUBCODE_LISTEN}/ws`;
...
async function main() {
  ...
  console.log("==================================================");
  console.log(`HUBCODE_HOME: ${HUBCODE_HOME}`);
