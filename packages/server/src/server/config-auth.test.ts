...
async function createHubcodeHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "hubcode-config-auth-"));
  roots.push(root);
  const hubcodeHome = path.join(root, ".hubcode");
  await mkdir(hubcodeHome, { recursive: true });
  await writeFile(path.join(hubcodeHome, "config.json"), JSON.stringify(config, null, 2));
  return hubcodeHome;
}
...
describe("daemon auth config", () => {
  ...
  test("loads optional auth password hash from config.json", async () => {
    const hubcodeHome = await createHubcodeHome({
      version: 1,
      daemon: {
  ...
  test("lets HUBCODE_PASSWORD override config.json auth password hash", async () => {
    const hubcodeHome = await createHubcodeHome({
      version: 1,
      daemon: {
        auth: { password: CONFIG_PASSWORD_HASH },
