...
interface WorkerTerminalRecord {
  ...
  // Live prompt state, kept out of `info`: re-registering a record (the
...
export function createWorkerTerminalManager(
  ...
  function handleWorkerEvent(message: TerminalWorkerToParentMessage): void {
    switch (message.type) {
      ...
      case "terminalPromptState": {
        handleTerminalPromptStateEvent(message);
        return;
      }
