...
`;
  ...
  it("returns lightweight checkout status for .hubcode worktrees", async () => {
    const result = await createWorktree({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    const status = await getCheckoutStatusLite(result.worktreePath, { hubcodeHome });
    expect(status.isGit).toBe(true);
  ...
  it("returns merged PR status when no open PR exists for the current branch", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '  echo \'{"url":"https://github.com/hubtool/hubcode/pull/123","title":"Ship feature","state":"closed","baseRefName":"main","headRefName":"feature","mergedAt":"2026-02-18T00:00:00Z"}\'',
        "  exit 0",
        "fi",
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        'args="$*"',
        'if [[ "$args" == "pr view --json url,title,state,baseRefName,headRefName,mergedAt" ]]; then',
        '  echo \'{"url":"https://github.com/hubtool/hubcode/pull/999","title":"Closed without merge","state":"closed","baseRefName":"main","headRefName":"feature","mergedAt":null}\'',
        "  exit 0",
        "fi",
        'echo "unexpected gh args: $args" >&2',
        "exit 1",
  ...
  it("caches PR status results for duplicate lookups", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '  printf "%s\\n" "$((count + 1))" > "$count_file"',
        '  echo \'{"url":"https://github.com/hubtool/hubcode/pull/123","title":"Ship feature","state":"OPEN","baseRefName":"main","headRefName":"feature","mergedAt":null}\'',
        "  exit 0",
        "fi",
        'echo "unexpected gh args: $args" >&2',
        "exit 1",
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '  next="$((count + 1))"',
        '  printf "%s\\n" "$next" > "$count_file"',
        '  printf \'{"url":"https://github.com/hubtool/hubcode/pull/%s","title":"Ship feature","state":"OPEN","baseRefName":"main","headRefName":"feature","mergedAt":null}\\n\' "$next"',
        "  exit 0",
        "fi",
        'echo "unexpected gh args: $args" >&2',
        "exit 1",
  ...
  it("throws if Hubcode worktree base metadata is missing", async () => {
    const worktree = await createWorktree({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    await expect(mergeToBase(worktree.worktreePath, {}, { hubcodeHome })).rejects.toThrow(/base/i);
  });
