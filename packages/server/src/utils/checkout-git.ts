...
export type CheckoutStatusGitNonHubcode = {
  isGit: true;
  repoRoot: string;
...
export type CheckoutStatusGitHubcode = {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusLiteGitNonHubcode = {
  isGit: true;
  currentBranch: string | null;
  remoteUrl: string | null;
  worktreeRoot: string;
...
export type CheckoutStatusLiteGitHubcode = {
  isGit: true;
  currentBranch: string | null;
  remoteUrl: string | null;
  worktreeRoot: string;
...
export type CheckoutStatusLiteResult =
  ...
  | CheckoutStatusLiteGitNonHubcode
  | CheckoutStatusLiteGitHubcode;
...
type ConfiguredBaseRefForCwd =
  | { baseRef: null; isHubcodeOwnedWorktree: false }
  | { baseRef: string; isHubcodeOwnedWorktree: true };
...
async function getConfiguredBaseRefForCwd(
  ...
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { baseRef: null, isHubcodeOwnedWorktree: false };
  }
  ...
  return {
    baseRef: requireHubcodeWorktreeBaseRefName(worktreeRoot),
    isHubcodeOwnedWorktree: true,
  };
}
...
export async function getCheckoutStatus(
  ...
  if (configured.isHubcodeOwnedWorktree) {
...
export async function getCheckoutStatusLite(
  ...
  if (inspected.configured.isHubcodeOwnedWorktree) {
    return {
      isGit: true,
...
export async function getCheckoutDiff(
  ...
  if (compare.mode === "uncommitted") {
    ...
    if (configured.isHubcodeOwnedWorktree && compare.baseRef && compare.baseRef !== baseRef) {
...
export async function mergeToBase(
  ...
  if (configured.isHubcodeOwnedWorktree && options.baseRef && options.baseRef !== baseRef) {
...
export async function createPullRequest(
  ...
  if (configured.isHubcodeOwnedWorktree && options.base && options.base !== base) {
