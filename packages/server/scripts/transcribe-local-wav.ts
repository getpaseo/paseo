...
function usage(): string {
  return [
    ...
    "",
    "Env fallbacks:",
    "  HUBCODE_LOCAL_MODELS_DIR, HUBCODE_LOCAL_STT_MODEL",
  ].join("\n");
}
...
function parseArgs(argv: string[]): CliOptions {
  ...
  const defaultModelsDir =
    process.env.HUBCODE_LOCAL_MODELS_DIR ?? path.join(hubcodeHome, "models", "local-speech");

  const positional: string[] = [];
  let outPath: string | undefined;
  let model = LocalSttModelIdSchema.parse(
    process.env.HUBCODE_LOCAL_STT_MODEL ?? DEFAULT_LOCAL_STT_MODEL,
  );
