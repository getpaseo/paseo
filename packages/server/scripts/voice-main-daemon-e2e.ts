...
async function main(): Promise<void> {
  const daemonUrl = process.env.HUBCODE_DAEMON_URL ?? "ws://localhost:6767".replace(/^ws/, "http");
  const timeoutMs = Number(process.env.HUBCODE_VOICE_E2E_TIMEOUT_MS ?? "120000");
