...
async function test_wait_for_permission_request(agentId: string): Promise<void> {
  ...
  while (Date.now() - startTime < maxWait) {
    const result = await ctx.hubcode(["permit", "ls", "--json"]);
  ...
  const statusResult = await ctx.hubcode(["inspect", agentId]);
...
async function test_permit_allow(agentShortId: string, requestId: string): Promise<void> {
  ...
  const result = await ctx.hubcode(["permit", "allow", agentShortId, requestId]);
