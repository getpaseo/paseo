 ...
 *
 * Tests the worktree commands for managing Hubcode-managed git worktrees.
...
try {
  // Test 1: worktree --help shows subcommands
  {
    ...
    const result = await $`npx hubcode worktree --help`.nothrow();
  ...
  // Test 2: worktree ls --help shows options
  {
    ...
    const result = await $`npx hubcode worktree ls --help`.nothrow();
  ...
  // Test 3: worktree ls handles daemon not running
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode worktree ls`.nothrow();
  ...
  // Test 4: worktree ls with --host flag is accepted
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode worktree ls --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: worktree archive --help shows options
  {
    ...
    const result = await $`npx hubcode worktree archive --help`.nothrow();
  ...
  // Test 6: worktree archive requires name argument
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode worktree archive`.nothrow();
  ...
  // Test 7: worktree archive handles daemon not running
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode worktree archive test-worktree`.nothrow();
    // Should fail because daemon not running
    assert.notStrictEqual(result.exitCode, 0, "should fail when daemon not running");
    const output = result.stdout + result.stderr;
    const hasError =
  ...
  // Test 8: worktree archive with name and --host flag is accepted
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode worktree archive test-worktree --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
    assert(!output.includes("unknown option"), "should accept --host flag");
    assert(!output.includes("error: option"), "should not have option parsing error");
  ...
  // Test 9: -q (quiet) flag is accepted with worktree ls
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode -q worktree ls`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 10: --json flag is accepted with worktree ls
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode worktree ls --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 11: hubcode --help shows worktree subcommand
  {
    console.log("Test 11: hubcode --help shows worktree subcommand");
    const result = await $`npx hubcode --help`.nothrow();
    ...
    console.log("✓ hubcode --help shows worktree subcommand\n");
  }
} finally {
