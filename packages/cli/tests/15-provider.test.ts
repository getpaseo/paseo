...
try {
  // Test 1: provider --help shows subcommands
  {
    ...
    const result = await ctx.hubcode(["provider", "--help"]);
  ...
  // Test 2: provider ls lists all providers
  {
    ...
    const result = await ctx.hubcode(["provider", "ls"]);
  ...
  // Test 3: provider ls --json outputs valid JSON
  {
    ...
    const result = await ctx.hubcode(["provider", "ls", "--json"]);
  ...
  // Test 10: provider models --quiet outputs model IDs only
  {
    ...
    const result = await ctx.hubcode(["provider", "models", "claude", "--quiet"]);
    assert.strictEqual(result.exitCode, 0, "should exit 0");
    const lines = result.stdout.trim().split("\n").filter(Boolean);
    assert.strictEqual(
      lines.length,
