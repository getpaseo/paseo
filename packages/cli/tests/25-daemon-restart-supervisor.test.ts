...
const hubcodeHome = await mkdtemp(join(tmpdir(), "hubcode-restart-supervisor-"));
...
try {
  ...
  const statusBeforeRestart = await readDaemonStatus(hubcodeHome);
  ...
  const statusAfterRestart = await readDaemonStatus(hubcodeHome);
  assert.strictEqual(
