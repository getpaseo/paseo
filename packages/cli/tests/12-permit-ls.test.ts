...
try {
  // Test 1: permit --help shows subcommands
  {
    ...
    const result = await $`npx hubcode permit --help`.nothrow();
  ...
  // Test 2: permit ls --help shows options
  {
    ...
    const result = await $`npx hubcode permit ls --help`.nothrow();
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`HUBCODE_HOST=localhost:${port} HUBCODE_HOME=${hubcodeHome} npx hubcode -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
