...
try {
  {
    ...
    const paused = await ctx.hubcode(["schedule", "pause", createdJson.id, "--json"]);
    assert.strictEqual(paused.exitCode, 0, paused.stderr);
    assert.strictEqual(JSON.parse(paused.stdout).status, "paused");

    const resumed = await ctx.hubcode(["schedule", "resume", createdJson.id, "--json"]);
  ...
  await ctx.stop();
  await rm(ctx.hubcodeHome, { recursive: true, force: true });
