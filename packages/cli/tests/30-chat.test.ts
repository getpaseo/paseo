...
try {
...

  {
    ...
    const posted = await ctx.hubcode(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { HUBCODE_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.hubcode(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.hubcode(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    const waitPromise = ctx.hubcode(["chat", "wait", "coord-room", "--timeout", "5s"]);
    await new Promise((resolve) => setTimeout(resolve, 250));
    const secondPost = await ctx.hubcode(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.hubcode(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.hubcode(["chat", "delete", "coord-room"]);
