...
import { resolveLocalHubcodeHome } from "./local-daemon.js";
...
export async function setDaemonPasswordInConfig(
  ...
  const configPath = path.join(hubcodeHome, CONFIG_FILENAME);
  ...
  savePersistedConfig(hubcodeHome, nextConfig);

  return {
    action: "password_set",
    configPath,
    restartCommand: "hubcode daemon restart",
    message: `Password written to ${configPath}\nRestart the daemon for the change to take effect.\nRun: hubcode daemon restart`,
  };
}
