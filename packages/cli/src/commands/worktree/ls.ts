...
export function resolveHubcodeHomePath(): string {
  return process.env.HUBCODE_HOME ?? join(homedir(), ".hubcode");
}

export function resolveHubcodeWorktreesDir(): string {
  return join(resolveHubcodeHomePath(), "worktrees");
}

function isAgentInManagedWorktree(agentCwd: string): boolean {
  const worktreesDir = resolveHubcodeWorktreesDir();
...
export async function runLsCommand(
  ...
  try {
    ...
    const response = await client.getHubcodeWorktreeList({});
