...
import { loadConfig, resolveHubcodeHome, DaemonClient } from "@gethubcode/server";
...
const PID_FILENAME = "hubcode.pid";
...
function readPidSocketTarget(hubcodeHome: string): string | null {
  const pidPath = path.join(hubcodeHome, PID_FILENAME);
...
function resolveConfiguredIpcDaemonHost(env: NodeJS.ProcessEnv, hubcodeHome: string): string | null {
  ...
  const pidHost = normalizeDaemonHost(readPidSocketTarget(hubcodeHome) ?? "");
  if (isIpcDaemonHost(pidHost)) {
    return pidHost;
  }

  const config = loadConfig(hubcodeHome, { env });
...
export function resolveDefaultDaemonHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const hubcodeHome = resolveHubcodeHome(env);
  const candidates: string[] = [];
  const configuredIpcHost = resolveConfiguredIpcDaemonHost(env, hubcodeHome);
  if (configuredIpcHost) {
    candidates.push(configuredIpcHost);
  }
  const configuredTcpHost = resolveConfiguredTcpDaemonHost(env, hubcodeHome);
...
export async function connectToDaemon(options?: ConnectOptions): Promise<DaemonClient> {
  ...
  async function tryNext(index: number, lastError: unknown): Promise<DaemonClient> {
    if (index >= hosts.length) {
      ...
      throw new Error(`Unable to connect to Hubcode daemon via ${hosts.join(", ")}`);
    }
