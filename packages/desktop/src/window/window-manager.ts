...
export function registerWindowManager(): void {
  ...
  ipcMain.handle("hubcode:window:toggleMaximize", (event) => {
  ...
  ipcMain.handle("hubcode:window:isFullscreen", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    return win?.isFullScreen() ?? false;
  });

  ipcMain.handle("hubcode:window:setBadgeCount", (_event, count?: unknown) => {
  ...
  ipcMain.handle("hubcode:window:updateWindowControls", (event, update?: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) {
      return;
    }
...
export function setupWindowResizeEvents(win: BrowserWindow): void {
  win.on("resize", () => {
    win.webContents.send("hubcode:window:resized", {});
  });
