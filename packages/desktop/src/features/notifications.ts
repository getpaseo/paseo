...
export function registerNotificationHandlers(): void {
  ipcMain.handle("hubcode:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("hubcode:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("hubcode:event:notification-click", payload);
      }
