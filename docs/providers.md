...
ACP cancellation treats a resolved `session/cancel` request as the provider's interrupt acknowledgement. The adapter terminalizes its local foreground turn at that point even if the outstanding `session/prompt` request remains blocked behind a permission or question UI. Every cancellation source for the same turn, including concurrent interrupts and interrupting permission denials, shares one cancel request, and synthesized canceled tool snapshots remain terminal. Any later resolution or rejection from the old prompt is ignored by turn ID so stale work cannot terminate or leak into a replacement turn.
