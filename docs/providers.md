...
OMP supports native Paseo host tools through `set_host_tools`. The adapter registers the complete caller-scoped catalog with `loadMode: "discoverable"`, retaining names, schemas, callbacks, progress, cancellation, injection controls, and permissions. OMP can present those tools under `xd://`; disabling xdev leaves normal top-level presentation. Discovery does not remove capabilities or require a replacement MCP server. OMP-managed task children still surface through `child_session` imports while their runtime remains owned by OMP. Custom OMP profiles should extend `omp`; other Pi-compatible forks can extend `pi`, override `command`, and set their JSONL session directory.

OMP todos use full authoritative snapshots from successful native/device results and `get_state.todoPhases`. Reminder events contain subsets and must not replace the current list. Absent or malformed snapshots leave state unchanged; an explicit empty snapshot clears it. Optional `state`, `phase`, `phaseIndex`, and `blocker` fields retain native semantics across stream conversion, history, and cache reload. Keep the legacy three-state wire projection parseable: blocked remains pending there, and abandoned remains terminal. Updated consumers use the richer state and do not count abandoned work as successful completion.
