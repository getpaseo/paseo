...
Pi MCP support depends on the open-source `pi-mcp-adapter` extension being loaded for the agent cwd. Probe with Pi RPC `get_commands`; the adapter registers an extension command named `mcp` (often with `sourceInfo.source` containing `pi-mcp-adapter`). Paseo injects its own `/mcp/agents` server by default; `daemon.mcp.injectIntoAgents: false` is an explicit opt-out. When Paseo injects MCP servers into Pi, write a per-agent MCP config and pass it with `--mcp-config` instead of modifying user or project MCP files. Refresh auto-injected `/mcp/agents` URLs on resume/reload so dev daemons with ephemeral ports do not leave stale MCP endpoints in persisted agent config. For local HTTP servers such as Paseo's own `/mcp/agents` endpoint, explicitly disable adapter OAuth (`auth: false`, `oauth: false`) in the generated config.
