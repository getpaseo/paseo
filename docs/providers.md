...
OMP may deliver a late terminal event during an interrupt-and-replace boundary and briefly lose the client metadata on the following user echo. For live OMP events only, the manager may reconcile an untagged echo when exactly one canonical submitted row has the same foreground `turnId`, exact text, and wire identity; it records the native provider ID internally and suppresses the duplicate wire row. This is deliberately turn-scoped so provider-owned messages and repeated prompts on different turns remain visible.
