...
- Model the stop as an explicit `stopping` turn-state variant that carries the cancel promise and the canceled run's terminal. One stop owns every cancel it issues, including retries — never fire a detached one, it will outlive its boundary. Pressing Stop again retries the stop already in progress; its settlement accumulates, because an earlier request still in flight can land on a replacement just as easily as the newest.
