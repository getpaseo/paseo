...
The catalog also merges in Anthropic models from https://models.dev/api.json that the manifest does not spell yet, so a new release is selectable before a Paseo update ships (`model-discovery.ts`). Discovered rows carry only the standard effort levels and no feature gates; shipping the manifest entry is still what lands a model properly, and a manifest id always wins over a discovered one. The response is cached for 24h at `$PASEO_HOME/cache/claude-models.json` and reused past that when the fetch fails. `PASEO_CLAUDE_MODEL_DISCOVERY=off` disables the fetch.
