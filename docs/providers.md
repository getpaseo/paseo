...
OpenCode provider-native child sessions are eagerly adopted as Paseo subagents. The adapter records which OpenCode helper server spawned each child and resumes adopted children on that same server, rather than starting a fresh dedicated server, so live global events and HITL permissions continue streaming. Provider deletion events archive the adopted child. The parent `sub_agent` timeline item is only a pointer with `childSessionId`; child tool/action logs belong to the adopted child agent.
