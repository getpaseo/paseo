...
The catalog also merges Anthropic models from models.dev that the manifest does not spell yet, so a new release is selectable before a Paseo update ships (`model-discovery.ts`). Discovered rows carry no curated capabilities; shipping the manifest entry is still what lands a model properly, and a manifest id always wins over a discovered one.
