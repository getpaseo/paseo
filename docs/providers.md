...
Active-turn steering is an optional `AgentSession.steerActiveTurn` operation. The manager owns admission against its exact foreground turn, canonical user-message creation, echo reconciliation, and falls back to the normal interrupt-and-replace path only when the adapter reports `unavailable`. An adapter error leaves the steer's fate ambiguous and must surface without an interrupt or retry. Codex calls `turn/steer` with the native expected turn and Paseo client user-message ID. Claude pushes an admitted steer into the exact active SDK query input; isolated control commands remain unavailable. OpenCode calls `session/prompt_async` with an OpenCode-generated message ID; the server queues the prompt while busy and the next LLM call in the same Paseo turn includes it. Pi sends its native `steer` RPC, which queues the message for delivery after the in-flight assistant turn's tool calls. Slash-command inputs report `unavailable` because pi rejects extension commands on the steer path, and echo identity is correlated by message text because pi's steer RPC takes no message ID. A Generic ACP provider may opt into concurrent-prompt steering when its agent redirects a second `session/prompt` into the active turn; Hermes enables that transport by default. ACP prompt completion acknowledges the end of the turn rather than admission, so the adapter reports acceptance after dispatch and logs a later prompt failure without interrupting or replacing work whose fate is already ambiguous. A missing session reports `unavailable` and uses the normal interrupt fallback.

A steering adapter also owes its interrupt: stopping a turn must discard the steers the provider has not read yet, or one of them resumes the turn the user just stopped. Codex clears pending input when it aborts a turn; Claude does not, so its adapter cancels the SDK messages it queued before calling `query.interrupt()`. Pi requires `clear_queue` before `abort`; older binaries without that RPC retain their native queue behavior until the pi compatibility floor reaches 0.84.4. Concurrent ACP steering invalidates its admitted steer generation before sending `session/cancel`, so late prompt settlement cannot complete or resume a later foreground turn.

`SteerActiveTurnOptions.clearPendingPermissions` makes permission release part of the provider contract. Providers clear permissions before dispatch unless their native steer operation positively acknowledges queueing independently of turn completion. Concurrent ACP steering denies blocking permissions before dispatching `session/prompt`; a denial failure sends nothing, so the caller can safely retry. Pi may queue first because its native `steer` RPC acknowledges queueing. Steers without the flag leave permissions open. A denied plan remains in the timeline because the pending card was the only other copy of its text.
