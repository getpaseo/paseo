...
OpenCode helper servers must be launched from the agent or request cwd, not from the user's home directory. OpenCode boots location services and file-search indexing relative to the server process cwd before Paseo sends the per-request `directory`; using home can trigger home-wide watchers/indexing and make runaway helper CPU harder to attribute. Managed helper records should include the scoped cwd. Dedicated per-agent helper records should also include agent id and session id when available; shared cwd helpers must stay cwd-only so stale attribution does not follow later sessions.
