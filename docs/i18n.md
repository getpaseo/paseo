...
Follow the [personal fork policy](../CLAUDE.md#prioritize-this-personal-fork). Maintain English client-owned UI copy: labels, buttons, empty states, confirmation text, and local status/error wrappers. Existing non-English resources can remain incomplete; missing keys fall back to English.
...
The resource tests allow missing non-English keys, verify English fallback, and check that existing translations use known keys and preserve interpolation placeholders.
...
Within a migrated surface, keep client-owned copy in the English resources. Mixed-language labels are expected when an incomplete locale falls back to English.
