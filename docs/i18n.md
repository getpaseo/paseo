...
The persisted app language setting is `"system" | "ar" | "en" | "es" | "fr" | "ko" | "ru" | "zh-CN"`. `"system"` follows the device or browser locale when it maps to a supported locale; unsupported system locales fall back to English.
...
English source strings live in `packages/app/src/i18n/resources/en.ts`. Korean strings live in `packages/app/src/i18n/resources/ko.ts`. Simplified Chinese strings live in `packages/app/src/i18n/resources/zh-CN.ts`.
...
- Korean was added as `ko` with translated core app chrome for app shell, workspace, sidebar, Git diff, new workspace, and settings language flows. The Korean resource keeps English fallback strings for deeper screens that have not been translated yet while preserving full key parity.
