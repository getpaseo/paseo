...
The persisted app language setting is `"system" | "ar" | "en" | "es" | "fa" | "fr" | "ja" | "ko" | "pt-BR" | "ru" | "zh-CN"`. `"system"` follows the device or browser locale when it maps to a supported locale; unsupported system locales fall back to English. Persian maps from system locales `fa` and Persian regional locales. Japanese maps from system locales `ja` and Japanese regional locales. Korean maps from system locales `ko` and Korean regional locales. Brazilian Portuguese maps from system locales `pt-BR` and bare `pt`; other Portuguese regional locales remain unsupported until explicitly added.
...
- Batch 5C added Persian (`fa`) resource coverage, language selector labels, i18next registration, and system-locale mapping for `fa` and Persian regional locales. Persian does not enable RTL layout direction in this batch.
