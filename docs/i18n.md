...
### French editorial conventions

Use natural French action verbs for buttons: `Retour` for Back, `Accueil` for Home, `branche` for a Git branch, and `PR`/`MR` for change requests. Keep `workspace`, `worktree`, `daemon`, and `commit`, along with brands, commands, paths, and user content.

Write French values explicitly rather than importing English values or spreading English resource objects. Preserve interpolation tokens exactly, separate them from adjacent words, and use French spacing around punctuation. Review each value against its English source; key parity alone does not catch mistranslations.
