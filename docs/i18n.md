...
- Batch 4Z expanded the supported locale set to the six UN official languages: Arabic, Chinese, English, French, Russian, and Spanish. Arabic, French, Russian, and Spanish resources currently preserve the English source copy except for language-selector labels so the locale plumbing, persistence, system-locale matching, and key parity are in place before full translation polish. Arabic does not enable RTL layout direction in this batch.
