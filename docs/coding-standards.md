...
- React Compiler is on (`app.config.js` → `experiments.reactCompiler`). It memoizes render output against the inputs it can see, so a component that derives what it renders from something the compiler cannot track — `Date.now()`, a module-scope `Map`, any mutable read during render — will render once and then freeze, no matter how often it re-renders. Ticking components must keep the clock in state (`const [nowMs, setNowMs] = useState(...)`, the `LiveElapsed` shape), never a discarded tick counter (`const [, setTick]`), because a discarded counter is not an input to anything. When the render genuinely must read a mutable source, add `"use no memo"` with a comment saying which source and why. The failure is silent and looks like a dead timer.
