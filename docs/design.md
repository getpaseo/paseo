...
Window controls are header content, not a compact-layout condition. On Windows and Linux the app draws them itself, so the surface that owns the window's top-right corner mounts them inside its own header row and nothing reserves a floating band. macOS is the exception: the OS draws traffic lights over the renderer, so the surface owning the top-left corner still clears them. Rendered surfaces declare which top corners they physically occupy; full-window overlays redeclare both. A focused split pane owns both corners; if focus restoration temporarily exposes the full split tree, the split boundary takes the top strip instead of handing a corner to an arbitrarily narrow leaf. The 720px desktop breakpoint preserves the default 320px sidebar and target 400px center width when the Explorer is closed; it is product policy, not a corner rule. Which surface owns which corner, and what the app-drawn controls cost, is in [window-chrome.md](window-chrome.md).
