...
- **Daemon** — Local Hubcode server process; identified by `serverId`. UI: "Daemon" (system contexts only). Code: `serverId` (`packages/server/src/shared/messages.ts:1886`), `DaemonClient` (`packages/server/src/client/daemon-client.ts`).
...
- **Worktree** — Hubcode-managed git worktree (`~/.hubcode/worktrees/{name}`); also a `workspaceKind` value. UI: CLI + `hubcode.json` keys (`worktree.setup`, `worktree.teardown`) only. Code: `ProjectCheckoutLiteGitHubcodePayload` (`packages/server/src/shared/messages.ts:2042`); CLI `hubcode worktree` (`packages/cli/src/commands/worktree/index.ts:8`). Forbidden: "Checkout" as a synonym.
...
- **Schedule** — Cron-style trigger that creates remote agents. UI: CLI only (`hubcode schedule`). Code: `ScheduleCreateRequest` (re-exported from `packages/server/src/shared/messages.ts`). Don't confuse with: Loop (iterative re-execution of one agent).
...
- **Conflict** — Two distinct senses; do NOT use the bare word in UI copy without qualifying which: (a) **stale-write conflict** on `hubcode.json` ("Config changed on disk", code `stale_project_config`, `packages/app/src/screens/project-settings-screen.tsx:576`); (b) **git merge conflict** (no current UI string).
