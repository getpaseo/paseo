...
const hubcodeHomeRoot = await mkdtemp(path.join(os.tmpdir(), "hubcode-test-"));
...
const daemon = await createHubcodeDaemon(
  {
...
For simpler cases, `createTestHubcodeDaemon` + `DaemonClient` handles temp dirs and port selection:
...
The test helper does **not** expose `providerOverrides`. Use `createHubcodeDaemon` directly when you need it (see quick start above).
