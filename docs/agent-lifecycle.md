...
Agent lifecycle status stays literal: a parent agent is `idle` when its own turn is idle, even if a child is running. The displayed state is derived and may differ. `waiting_on_subagent` is a client-only bucket (`packages/app/src/utils/sidebar-agent-state.ts`), never a wire status and never written back to the directory: an agent whose own bucket is `attention` (a finished turn) or `done` reads `waiting_on_subagent` while any managed descendant is `running` or `initializing`. A descendant blocked on permission surfaces as `needs_input`. A descendant that finished or failed leaves the parent's own bucket alone; its outcome stays in the subagents track. Detached descendants contribute nothing, and an archived descendant contributes nothing of its own — only a live child still hanging below it in a partial local view keeps it visible, so an idle historical child cannot pin a parent to waiting.

The derivation is parentage-based and feeds the agent tab and, through the workspace activity index, the workspace row, its sidebar status group, and the collapsed project badge. Where it reaches the workspace depends on what the daemon already folds in: a cross-workspace child is a root of its own workspace, so the parent's workspace has no daemon contribution and reads `waiting_on_subagent`; a same-workspace or provider-native running child already contributes `running` to the parent's workspace on the daemon, which outranks waiting, so that workspace row reads `Working` while the parent's tab reads `waiting_on_subagent`.
