...
Workspace archival fences relationship changes. A detach or parent-label mutation that has already
started finishes before archival classifies the workspace; once archival owns the workspace, a new
relationship mutation is rejected instead of racing the archive boundary.
