...
Without a fork release tag, prerelease metadata is ignored, so `0.1.102-beta.1` and `0.1.102` both produce `1102`. For fork releases, the EAS and GitHub APK workflows pass the tag to `app.config.js`, which derives `base version code * 1000 + fork number` for both Android and iOS; `v0.2.5-fork.1` therefore uses `versionCode` and `buildNumber` `2005001`.
