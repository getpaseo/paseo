...
Nothing in the host can take either object away from the guest: they live in the guest's realm, which is cross-origin and unreachable from the host document. The random id is the entire mitigation, so it comes from `crypto.getRandomValues` — `packages/app/index.ts` installs an `expo-crypto`-backed polyfill before any other import, so that branch is the one that runs on device. A guest that guesses wrong learns it guessed wrong, since a correct stamp draws an `init` back.
