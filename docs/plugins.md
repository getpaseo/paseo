...
The id is random, not a counter, and on Android that is the whole of the defence. react-native-webview registers its bridge with `addWebMessageListener(view, "ReactNativeWebView", Set.of("*"), …)` and its callback drops the `isMainFrame` argument, so `window.ReactNativeWebView` is injected into every frame — the sandboxed guest included. There a plugin can post to the host directly, never touching the relay that does the stamping, and the id is the only field it cannot fill in correctly. Nothing in the host can take that object away from the guest: it lives in the guest's realm, which is cross-origin and unreachable from the host document.
