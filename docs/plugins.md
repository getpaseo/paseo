...
- **`<script type="speculationrules">` is dropped in the same pass.** Every other non-JS `<script>` is carried over untouched, because a plugin's `application/json` block is data it reads back out. Speculation rules are not data: they name URLs, and the platform acts on them when the element is connected.
