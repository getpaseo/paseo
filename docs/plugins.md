...
So the plugin does not get child browsing contexts at all. The same script installs a `MutationObserver` that removes any `iframe`/`frame`/`object`/`embed`/`webview`/`fencedframe` as it is inserted, which lands before the child document starts parsing and therefore before its inline script runs. The selector is the set of elements that can become a browsing context, not the subset reachable today — `object`/`embed` are already dead to `object-src 'none'`, and a `<fencedframe>` can only be navigated from a `FencedFrameConfig`, which comes from `runAdAuction` or `sharedStorage` and neither is exposed on an opaque origin.

Four rounds of review found a door that observer does not watch, so it is now the last line rather than the only one. What sits above it:
...
- **`document.write`, `document.writeln`, `setHTMLUnsafe`, `setHTML`, `Document.parseHTMLUnsafe`, `Document.parseHTML`, and `XSLTProcessor` throw.** The invariant is that no same-realm path reaches the HTML _document_ parser, and that list has now been found short five separate times. `XSLTProcessor.transformToDocument` was the fourth: a stylesheet with `<xsl:output method="html"/>` builds its result tree through the document parser, so `<template shadowrootmode="closed">` in the stylesheet mints a closed root with no `attachShadow` call to wrap, and `adoptNode` walks it into the live document intact — a move, not a clone, so `clonable` never enters into it. `transformToFragment` is fragment parsing and does not honour declarative shadow DOM today; it is denied anyway, because that distinction is one Chromium version from moving and no plugin needs either. Denying an API is a policy; removing an element afterwards is a race.

  The fifth was the Sanitizer API's _safe_ half. Only the `Unsafe` names were denied, and `element.setHTML()` is the same document parser with a filter on the end: it honours declarative shadow roots, so `setHTML(markup, { sanitizer: { elements: ["template", …], attributes: ["shadowrootmode", …] } })` mints a real closed root on a connected element. The way back into it is `attachInternals().shadowRoot`, which returns a root the _parser_ made — that is what "available to element internals" is for — so a custom element gets a handle to a closed root, appends an `<iframe srcdoc>` inside it, and no sweep can ever see it: a `MutationObserver` on `document` is not told about mutations in a shadow tree, and `Element.shadowRoot` reads `null`. Measured in round 13 with ICE gathering started from that child realm. Do not rely on the sanitiser stripping the payload — its filter list is its own to change, and the closed root is the primitive.
...
And three things the observer itself has to get right, each found by breaking an earlier version:
...
- **Watch attributes, not only insertions.** Round 15: the selector is not a list of tag names. Whether a `<script>` is speculation rules is decided by an attribute, and a sweep only ran when a node was _inserted_ — so a script appended as `application/json` was swept, kept as data, and given `type="speculationrules"` a tick later, after which nothing looked at it again. One `setAttribute` defeated the whole entry. The observer takes `attributes: true` now and re-matches each record's own target; the subtree did not change, so that is one `matches` rather than a second sweep. `attributeFilter` would narrow it and cannot be used — it is a `sequence<DOMString>`, so converting it walks the array's iterator, which the plugin owns, and one poisoned `Symbol.iterator` stops `observe` running at all.
...
### `<webview>` is a child realm the CSP cannot describe

Electron's `<webview>` is the worst of the embedding elements, and it exists only on the desktop build, where the renderer runs with `webviewTag: true`. A guest is a separate `WebContents` with its own session: the plugin document's CSP does not describe it, the iframe's `sandbox` flags are not inherited by it, and the neuter script never runs in it. It is the fresh realm the rest of this section is about, with a full network stack attached and no `frame-src` to say no.

It is in both denial selectors — the detached markup sanitiser and the observer sweep — alongside `iframe` and the rest.

**A plugin could not actually have attached one**, which is worth writing down because it is the difference between a closed hole and a guarded one. Measured against `electron@41.2.0` with a real window (`webviewTag: true`, `contextIsolation: true`), a local HTTP server counting hits, and `will-attach-webview` logged: a `<webview>` in the top frame attaches and fetches from both the markup and the script path, and a `<webview>` inside an `<iframe sandbox="allow-scripts" srcdoc>` does neither — no attach event, no request. It is not the `sandbox` attribute doing it: an unsandboxed `srcdoc` subframe fails the same way, and so does one with `nodeIntegrationInSubFrames: true`. In this version the element is main-frame-only.

So the selector entry is depth, not the thing standing between a plugin and the network. It stays, because "Electron only registers this in the main frame" is an implementation detail of one version with no spec behind it, and this branch's history is a list of controls that were fine until the version underneath them moved.

Electron's `will-attach-webview` gate is a companion, not a substitute, and it is worth knowing why. It checks the destination and the partition, and the partition name it requires is a constant in published source, so a plugin that got the element connected would satisfy it. Gating on the initiating frame instead is what you would want, and Electron does not offer it: `will-attach-webview` carries no `WebFrameMain`. Requiring the app to announce an attach over IPC first would work, but the announcement is async and the attach is synchronous on connection, so it means restructuring the browser pane's mount path — a change to a shipped feature, in exchange for depth behind a control that already holds. Not done, on purpose.
...
- **Plugin markup is sanitised while it is still detached.** `insertAdjacentHTML` goes into a `<div>` that is not in the document, where nothing loads and no hint fires; every `link`, `iframe`, `frame`, `object`, `embed`, `webview` and `fencedframe` is removed there, and only then is the subtree connected — as one fragment, so scripts still see their own siblings. Nothing legitimate is lost: CSP denies every resource a `<link>` could fetch anyway. This does not reach inside a `<template>`, whose content is a separate fragment `querySelectorAll` does not descend into — a hint parked there and cloned later by a script is the residual below, not a second hole.
- **`<script type="speculationrules">` is dropped in the same pass.** Every other non-JS `<script>` is carried over untouched, because a plugin's `application/json` block is data it reads back out. Speculation rules are not data: they name URLs, and the platform acts on them when the element is connected. This is the one entry in either selector decided by an attribute rather than a tag name, which is why the observer watches attribute changes too — see above.
...
Every message the native relay sends up carries the id of the document it came from, and the host drops anything stamped with a document it has already left. Web gets this for free — `frame.web.ts` compares `event.source` against a fresh iframe object per plugin — but a WebView swaps documents asynchronously and gives the host no source to compare, so a plugin still alive after a swap could otherwise settle the incoming plugin's handshake and have the host answer with an `init` carrying the new file's contents. Swapping plugins is one way to get two live documents; the retry button on the ready-timeout card is the other, and it changes no HTML, so the id is minted per document rather than derived from the plugin.

The id is random, not a counter, because on **both** native platforms the guest can post to the host directly, never touching the relay that does the stamping — and then the id is the only field it cannot fill in correctly. Assume the guest has this channel everywhere native; the difference between the two platforms is only how it gets there.

|         | How the guest reaches the host                                 | Why it looks safe and is not                                                                                                                                                                                                |
| ------- | -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Android | `window.ReactNativeWebView.postMessage`                        | `addWebMessageListener(view, "ReactNativeWebView", Set.of("*"), …)` injects into every frame, and the listener discards `isMainFrame`                                                                                       |
| iOS     | `window.webkit.messageHandlers.ReactNativeWebView.postMessage` | the `window.ReactNativeWebView` shim _is_ `forMainFrameOnly:YES`, but the handler behind it is registered on the whole `WKUserContentController`, and `didReceiveScriptMessage` never reads `message.frameInfo.isMainFrame` |

Nothing in the host can take either object away from the guest: they live in the guest's realm, which is cross-origin and unreachable from the host document. The random id is the entire mitigation, so it comes from `crypto.getRandomValues` — `packages/app/index.ts` installs an `expo-crypto`-backed polyfill at startup, long before anything renders a plugin, so that branch is the one that runs on device. A guest that guesses wrong learns it guessed wrong, since a correct stamp draws an `init` back.

One consequence of the iOS route: `WKScriptMessage.body` is any JSON value, so `nativeEvent.data` can be an object even though react-native-webview types it as `string`. Anything reading a message must handle both.
...
No `iframe`, `frame`, `object`, `embed`, `webview`, or `fencedframe` either — they are removed as they are inserted. Use inline `<svg>` or `<img src="data:…">`. A `<script type="speculationrules">` block is dropped for the same reason.

And `document.write`, `document.writeln`, `element.setHTML()`/`setHTMLUnsafe()`, `Document.parseHTML()`/`parseHTMLUnsafe()`, and both `XSLTProcessor` transform methods throw. Build with `innerHTML`, `insertAdjacentHTML`, or DOM calls.
...
**Sidebar panel.** One extra tab in the explorer sidebar next to Changes, Files, and PR, per declared panel — a manifest may contribute several. Two panels sharing an `id` collapse to the first and nothing rejects the manifest, so keep the ids distinct. Every plugin tab renders the same generic icon; the manifest's `icon` is parsed but not honoured, because honouring it means bundling the whole icon set to satisfy a name only known at runtime.
...
- **`sha256` and `bytes` both have to match**, and the download is aborted mid-stream once it passes `bytes` — a CDN that streams more than the entry declares fails the install rather than filling the daemon's heap. `sha256` is lowercase hex; `shasum -a 256 <file>` and `wc -c < <file>` give you both. Re-hash and re-measure on every publish; a stale hash is the most common broken listing.
...
- **A whole install has 60 seconds, not 30 seconds per file.** The per-request timeout is per request, so 64 files that each stall would outlast the client's own 120s RPC timeout many times over. Host the files somewhere that answers.
...
All six were found broken during review, across twelve rounds: a regex-placed CSP; an `originWhitelist` that bypassed our own navigation check; the self-navigation hole that survived the first round of fixes; WebRTC — reproduced against a real TURN server that received the file content verbatim; WebRTC again one frame deeper, in a `srcdoc` child the deletion never reached; the frame remover itself, poisoned through the prototypes it looked things up on and detached by `document.write()`; the frame remover again, blind to a frame inside a shadow root; a `<link>` resource hint, which resolves a hostname with no request for any CSP to check; three rounds of late binding in the shell itself, each one breaking the round before it; `XSLTProcessor`, a fourth way into the HTML document parser and therefore a fourth way to a closed shadow root; and the Sanitizer API's safe `setHTML`/`parseHTML`, a fifth, reached back into through `attachInternals().shadowRoot`. Round 15 was the observer's own blind spot: it only ever ran on insertion, so a `<script>` appended as `application/json` and retyped `speculationrules` one tick later was never looked at again. Every one passed typecheck, lint, and the full test suite. Round 11 added `<webview>` to the selectors and round 12 added `<fencedframe>` and the `speculationrules` drop, but those three are gaps rather than holes — no attack reached the network through any of them, and the `<webview>` measurement is above.

Note the shape of those. The round-three fix was itself incomplete. The test written to prove it asserted the wrong thing. The round-four fix could be disabled by the plugin from inside, and the round-five one could be walked around. A denial test has to be driven from where the attacker's code runs; a runtime mitigation has to assume the attacker will attack the mitigation; and control #3 got to seven rounds because it was a list of doors rather than a policy, which is why it is now a policy with the list underneath it.
...
- **`<link>`, `<iframe>`, `<frame>`, `<object>`, `<embed>`, `<webview>`, `<fencedframe>` and `<script type="speculationrules">` are removed**, silently and whatever the attributes say — except inside a `<template>`, whose content is a separate fragment the strip does not descend into. One parked there is inert until a script clones it out, at which point it is the residual above rather than a second way in. Nothing they could load is reachable — the CSP denies external stylesheets, scripts and frames, and even a `data:` stylesheet, so a `<link>` never worked here. Inline your CSS in a `<style>`. The reasons are under [Resource hints](#resource-hints-leak-a-hostname-and-one-of-them-is-not-closed) and above it.
