...
```bash
npm run build --workspace=@hubtool/relay
```

The Node daemon imports `@hubtool/relay` from `packages/relay/dist/*`, not `src/*`.
...
```bash
npm run build --workspace=@hubtool/server
```

The CLI imports `@hubtool/server` via package exports resolving to `dist/*`. Stale `dist` means the CLI speaks an old protocol and fails with handshake warnings or timeouts.
