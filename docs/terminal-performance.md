...
- **The pane reacts to the settled keyboard inset, never the per-frame one.** `TerminalPane` reads `useSettledKeyboardShift()`, which publishes once when keyboard motion stops, and pulses its refits from that. Subscribing to the live shift shared value (`useKeyboardShiftStyle().shift` or `useKeyboardShift()`) with `runOnJS` runs a state update, a resize/measure, and a clipboard bridge call on every frame of the ~250ms keyboard animation and stutters on phones. The padding animation itself stays on the UI thread.
