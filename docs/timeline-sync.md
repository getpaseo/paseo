...
The draft-create handoff has the same lifetime: the viewed-timeline owner releases it when it
reports the chat current, not when the first authoritative page lands. Releasing it at the first
page leaves a chat this client just created looking hydrated but still catching up, which shows
Updating messages for a conversation that cannot be out of date.
