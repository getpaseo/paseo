...
Ledger version 2 assigns each delivery a durable, per-owner sequence when the serialized owner mutation runs. New responses use `seq:<n>` cursors; old numeric sequence cursors remain readable during migration, while delivery IDs are reserved for exact lookup. A `dispatching` record loaded by a later daemon is marked `ambiguous` and is never retried automatically. Acknowledgement is valid only after `accepted`. Repeating acknowledgement of a retained acknowledged record is idempotent for a client that advertises `deliveryPayloadTombstones`; an older client may receive `delivery_payload_unavailable` after that record's payload has been compacted.

New SDK records use an exact `targetAgentId`. An old wire send that omits the target is durably accepted as `deliveryMode: "legacy_pull"` without native dispatch, so the public consumer can acknowledge it. Version-1 pull records without a target are migrated to the same state. Both forms retain their payload and old delivery identity. A loaded unacknowledged record must have a target and payload unless it is `legacy_pull`; payload fingerprints are recomputed before the record is admitted. Invalid identity, unsafe payload keys, excessive depth, and other malformed records are quarantined.

Pending record and byte quotas exclude acknowledged records. Acknowledged payloads admitted by a client advertising `deliveryPayloadTombstones` may be compacted by age, count, or bytes while their delivery ID and payload fingerprint remain as an idempotency tombstone until tombstone retention expires. A capable client receives that tombstone and can acknowledge it repeatedly. An older client never receives a compacted row through listing or lookup; acknowledging its known delivery ID may return `delivery_payload_unavailable` because its wire contract requires a payload. Unacknowledged records are never removed by this GC.
