...
| `autoUpdateTitle` | `boolean`                                       | Whether the daemon may asynchronously update `title` from conversation context. Defaults to `false` for existing workspaces and `true` for newly created ones.                        |
