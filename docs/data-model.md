...
| `labels`             | `Record<string, string>`                 | Key-value labels (default `{}`). `paseo.parent-agent-id` tracks parent/child agents; `paseo.workspace-id` pins an agent to a workspace when multiple workspaces share the same directory. See [agent-lifecycle.md](./agent-lifecycle.md) |
