...
| `autoArchivedChangeRequestUrl` | `string \| null`                                | Change request whose merged state triggered auto-archive. Restore preserves it, or records the current merged change request for legacy archives, so repeated snapshots cannot archive the workspace again. |
