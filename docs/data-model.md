...
> **Opaque-ID invariant:** `workspaceId` is opaque identity, never a filesystem path. Filesystem and git operations take `cwd`/`workspaceDirectory` only — never the id. A compatibility-only first-materialization bootstrap groups pre-registry agent records by exact source directory, using `mainRepoRoot` only to keep linked worktrees with their source clone. Git remotes are metadata, never grouping identity. Existing registries that predate this rule are repaired by the one-time compatibility migration described above.
