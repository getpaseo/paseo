...
| `setupStatus`                  | `"pending" \| "completed" \| "failed" \| null`  | Durable managed-worktree setup readiness. Null identifies workspaces created before readiness tracking or workspaces that require no managed setup.                                           |
| `setupError`                   | `string \| null`                                | Last managed-worktree setup failure. A persisted pending setup is restored as failed after daemon restart because the interrupted command cannot be resumed safely.                           |
