...
Ledger version 2 assigns each delivery a durable, per-owner sequence when the serialized owner mutation runs. New responses use `seq:<n>` cursors; old delivery-ID and numeric sequence cursors remain readable during migration, with exact delivery IDs resolved first. A `dispatching` record loaded by a later daemon is marked `ambiguous` and is never retried automatically. Acknowledgement is valid only after `accepted`, and repeated acknowledgement of a retained acknowledged record is idempotent.

New records require an exact `targetAgentId`. Version-1 pull records without a target are migrated to `deliveryMode: "legacy_pull"` and `accepted`, so the public consumer can acknowledge them. They retain their payload and old delivery identity. A loaded unacknowledged record must have both a target and payload; payload fingerprints are recomputed before the record is admitted. Invalid identity, unsafe payload keys, excessive depth, and other malformed records are quarantined.

Pending record and byte quotas exclude acknowledged records. Acknowledged payloads admitted by a client advertising `deliveryPayloadTombstones` may be compacted by age, count, or bytes while their delivery ID and payload fingerprint remain as an idempotency tombstone until tombstone retention expires. Responses to older clients omit compacted rows so they never receive a payloadless record where a payload is required. Unacknowledged records are never removed by this GC.
