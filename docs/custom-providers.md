...
Choose the mode when the agent is created. Switching it inside a running session answers with a notice instead of taking effect, because the value is fixed for the life of the process. An agent created without a mode inherits DSH's own default. An explicit `DSH_PERMISSION_MODE` in the provider or agent environment wins over the mode you picked, and the session then reports that preset rather than the one you picked. How a provider declares launch-time modes is in [providers.md](providers.md).
