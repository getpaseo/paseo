...
Claude profiles that pin an account with `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`) also get their own plan-usage entry in the context-meter tooltip and the host Usage page, under the profile's label. Anthropic's usage API requires the `user:profile` scope, which setup-tokens do not carry, so those profiles read their session and weekly windows from the rate-limit headers of a minimal one-token API call instead — spending a few tokens of the pinned account's own quota per refresh. The plan label exists only on the usage API, so it stays absent for setup-token profiles. Once an agent on the profile has made an API call, the context-meter tooltip prefers the windows Claude Code reported for that call (`rate_limit_event`), which include the per-model weekly window (e.g. `Weekly · Fable`) for the model the agent is running; the profile's fetched entry covers the host Usage page and agents that have not run yet. Profiles without a pinned token share the base `claude` entry, which reads the machine's active Claude login.
