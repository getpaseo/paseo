...
- omp gets a hook factory at `<agentDir>/hooks/post/paseo-terminal-activity.js`. Paseo resolves `agentDir` from the daemon env with omp's precedence: a named `OMP_PROFILE`/`PI_PROFILE` uses `~/.omp/profiles/<name>/agent`, the default profile uses `PI_CODING_AGENT_DIR` when set, otherwise `~/.omp/agent` (`PI_CONFIG_DIR` replaces `.omp` in both paths). A profile picked only at launch (`omp --profile x` or a profile alias) is invisible to the daemon, so that session does not load the hook.
