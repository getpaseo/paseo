...
`PaseoApi` is the capability-only boundary over workspaces, agents, providers, config, and
backend-owned durable deliveries. `PaseoPluginApi` is the scoped plugin facade and omits the
