...
>
> **Daemon-owned browser host.** `daemon.browserTools.cdpEndpoint` attaches the daemon to a CDP endpoint the operator runs and registers it as a browser host of its own, so a headless server mirrors without any desktop app. It is attach-only: Paseo never launches or bundles a browser. It advertises the viewer and mirror commands and deliberately not the ref-based ones, which need the snapshot engine the Electron host injects into its guests — `supportedCommands` is per-host and the broker refuses the rest here. `packages/server/src/server/browser-tools/cdp-connection.ts` is the seam the tests replace with an in-memory endpoint; the `Input.*` payload builders both hosts share live in `browser-tools/cdp-input.ts` and are re-exported to `packages/desktop` through the `@getpaseo/server/browser-tools/cdp-input` subpath.
>
> Chrome allows one screencast per page, so the daemon owns the capture rather than each viewer: it allocates a slot per mirrored browser, runs the capture at the largest size any viewer asked for, and re-arms it only when that maximum changes. Frames are addressed by slot, so slot reuse after a stream ends is the hazard to watch — a frame from a torn-down capture must never reach whoever holds that slot next.
