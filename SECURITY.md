...
One channel is not closed. `<link rel="preconnect">` resolves a hostname and opens a socket without making a request, so no CSP directive is consulted, and the hint fires on connection before anything we can install universally. Plugin _markup_ cannot carry one — it is sanitised while still detached — and `x-dns-prefetch-control: off` disables `dns-prefetch` document-wide, but a plugin _script_ can still cause one lookup to a host of its choosing. No web platform primitive denies it. An installed plugin is code you trust with the files you open in it, the same as a VS Code extension.
