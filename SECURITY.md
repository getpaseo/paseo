...
Any one of the six failing is an exfiltration path. All six did fail during this feature's review, across six rounds, and every one passed the full test suite — so changes to the sandbox need a test that proves the denial in a real browser, driven from where the attacker's code runs rather than from the host. The realm control is the one to keep in mind: three separate rounds found a door it did not watch — poisoned DOM methods, `document.write()` detaching it from the tree, then a frame hidden in a shadow root the observer is never notified about. It is now a policy (the plugin's markup never reaches the HTML parser, and the APIs that do are denied) with the frame removal underneath it, because a mitigation that enumerates doors loses to whoever finds the next one. [docs/plugins.md](docs/plugins.md) documents the sandbox and the bridge.
