...
Keeping that content on the machine depends on four separate controls: a CSP on the plugin that denies every outbound request, `frame-src 'none'` on the app's own document so the plugin frame cannot navigate itself out (the guest CSP cannot stop that — a frame's navigation is checked against its parent's policy), an iframe with no `allow-forms`/`allow-popups`/`allow-top-navigation` on web, and a deny-by-default navigation check on native. Any one of them failing is an exfiltration path — three did fail during this feature's review, across two rounds, and all three passed the test suite — so changes to the sandbox need a test that proves the denial in a real browser. [docs/plugins.md](docs/plugins.md) documents the sandbox and the bridge.
