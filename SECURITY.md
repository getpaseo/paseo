...
Plugins are third-party HTML rendered by the client. They run in a sandboxed frame with no storage and no origin, and they reach the app only through a fixed `postMessage` bridge. A plugin sees the content of files you preview with it and can ask the app to open a path inside the workspace; it cannot call the daemon or reach anything else. Messages the other way are source-checked, so no other frame on the page can drive the app through a plugin's channel.

Two plugins open at once are sibling frames in one document on web, and a sibling can `postMessage` directly into another plugin's frame — the host is not on that path and cannot filter it. It cannot read the victim's context or navigate it (the sandbox denies that without `allow-same-origin`), but it can send a well-formed `update` and make the victim render attacker content as if it were your file. The defence is the plugin's: check `event.source === window.parent`, which [docs/plugins.md](docs/plugins.md) requires and no host-side control can substitute for.

Keeping that content on the machine depends on six separate controls: a CSP on the plugin that denies the network requests CSP can express; deletion of `RTCPeerConnection` from the plugin's realm, because no CSP directive covers WebRTC and a plugin otherwise chooses both the ICE server and the payload; denial of every route to a fresh realm, because one is created with its own untouched `RTCPeerConnection` and cannot be injected into — plugin markup is inserted rather than parsed as the document, `document.write`/`setHTML`/`setHTMLUnsafe`/`parseHTML`/`parseHTMLUnsafe`/`XSLTProcessor` throw, `attachShadow` is wrapped so the roots it hands out are watched, and frames — including Electron's `<webview>` on the desktop build — are removed as they are inserted; `frame-src 'none'` on the app's own document so the plugin frame cannot navigate itself out (the guest CSP cannot stop that — a frame's navigation is checked against its parent's policy); an iframe with no `allow-forms`/`allow-popups`/`allow-top-navigation`/`allow-same-origin`; and a deny-by-default navigation check on native. Native loads that same sandboxed iframe inside a host document rather than running the plugin as the WebView's top document: an unsandboxed top document hands its own origin to any frame the plugin creates, which makes the child readable in the same task it is inserted, before any runtime removal can run.

One channel is not closed. `<link rel="preconnect">` resolves a hostname and opens a socket without making a request, so no CSP directive is consulted, and the hint fires on connection before anything we can install universally. Markup that renders cannot carry one — it is stripped while still detached — and `x-dns-prefetch-control: off` disables `dns-prefetch` document-wide, but a plugin script can still cause one lookup to a host of its choosing, including by cloning a `<link>` parked inside a `<template>`, whose content the strip does not descend into. No web platform primitive denies it. Measured against a server counting TCP connections, the runtime removal in fact beat the hint and no connection was made — the channel is documented as open regardless, because that is one browser's scheduling rather than a guarantee. An installed plugin is code you trust with the files you open in it, the same as a VS Code extension.

Any one of the six failing is an exfiltration path. All six did fail during this feature's review, across fourteen rounds, and every one passed the full test suite — so changes to the sandbox need a test that proves the denial in a real browser, driven from where the attacker's code runs rather than from the host. The realm control is the one to keep in mind: seven separate rounds found a door it did not watch — poisoned DOM methods, `document.write()` detaching it from the tree, a frame hidden in a shadow root the observer is never notified about, three rounds of the shell resolving something late enough for the plugin to have replaced it, `XSLTProcessor`, whose result tree is built by the HTML document parser and so honours a closed declarative shadow root, and the Sanitizer API's safe `setHTML`, which does the same and hands the closed root back through `attachInternals()`. The eighth was the `attachShadow` wrapper itself: `mode` is an enum the platform stringifies, so `new String("closed")` was not `"closed"` to the wrapper's strict comparison and was a valid closed root to the platform, retrieved through the same `attachInternals()`. Two more, Electron's `<webview>` and `<fencedframe>`, were missing from the selectors but neither can reach the network from the plugin frame — `<webview>` was measured not to attach from a subframe in the shipped Electron, and a `<fencedframe>` has no navigable config on an opaque origin; they are guarded rather than closed. It is now a policy (the plugin's markup never reaches the HTML parser, and the APIs that do are denied) with the frame removal underneath it, because a mitigation that enumerates doors loses to whoever finds the next one. [docs/plugins.md](docs/plugins.md) documents the sandbox and the bridge.
...
The registry is a trust boundary. The daemon verifies each downloaded file against the SHA-256 in the index, which protects the bytes in transit but says nothing about the publisher's intent — installing a plugin means trusting whoever published it. Downloads over HTTP are capped while streaming rather than after buffering, so a registry host that declares a small file and serves a huge one cannot exhaust daemon memory. Point `daemon.plugins.registryUrl` at your own index if you want to control what is installable.
