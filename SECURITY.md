...
Any one failing is an exfiltration path. Five did fail during this feature's review, across four rounds, and every one passed the full test suite — so changes to the sandbox need a test that proves the denial in a real browser, driven from where the attacker's code runs rather than from the host. [docs/plugins.md](docs/plugins.md) documents the sandbox and the bridge.
