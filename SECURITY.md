...
Keeping that content on the machine depends on five separate controls: a CSP on the plugin that denies the network requests CSP can express; deletion of `RTCPeerConnection` from the plugin's realm, because no CSP directive covers WebRTC and a plugin otherwise chooses both the ICE server and the payload; `frame-src 'none'` on the app's own document so the plugin frame cannot navigate itself out (the guest CSP cannot stop that — a frame's navigation is checked against its parent's policy); an iframe with no `allow-forms`/`allow-popups`/`allow-top-navigation`/`allow-same-origin` on web; and a deny-by-default navigation check on native.

Any one failing is an exfiltration path. Four did fail during this feature's review, across three rounds, and every one passed the full test suite — so changes to the sandbox need a test that proves the denial in a real browser, not a reading of the policy. [docs/plugins.md](docs/plugins.md) documents the sandbox and the bridge.
