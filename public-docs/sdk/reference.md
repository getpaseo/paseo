...
| `cancelTurn(expectedTurnId?)` | `PaseoAgentCancelTurnResult`      | Exact-turn cancellation when an ID is supplied; omitted IDs use daemon current-turn semantics.          |
...
| `acknowledge(deliveryId, options?)`      | `PaseoDelivery`          | Records an acknowledgement; capable clients may repeat it after payload compaction. Older clients may receive `delivery_payload_unavailable`. |

`payload` may be any bounded JSON value (64 KiB maximum; nested values and strings are also bounded). Targeted sends use `messageId` when supplied, or `deliveryId` as the stable provider-facing identity. A successful targeted send has status `accepted`; a known pre-dispatch failure has status `failed`; a provider or restart outcome that cannot be proven has status `ambiguous`. Ambiguous records are not retried automatically. Acknowledgement applies only to accepted deliveries; failed and ambiguous records remain available for diagnosis.

Delivery IDs are scoped to the authenticated principal. New SDK sends use the exact target agent ID. A targetless old wire send is durably accepted as `legacy_pull` without native dispatch, and version-1 pull records are migrated to the same state. Cursors returned by new hosts are `seq:<n>`; older numeric sequence cursors remain accepted. Plugin runtimes receive the scoped `PaseoPluginApi`, which does not expose `deliveries`; plugin-owned ledger state is isolated by installation and purged when the plugin socket closes. Targeted delivery requires `durableDeliveryTargeting`. Acknowledged payload compaction requires `deliveryPayloadTombstones`; capable clients can acknowledge a retained tombstone repeatedly, while older clients never receive a compacted payloadless row and may get `delivery_payload_unavailable` when acknowledging its ID. An accepted delivery can be acknowledged; failed and ambiguous records remain available for diagnosis. An older host rejects an unsupported call without sending an RPC.
